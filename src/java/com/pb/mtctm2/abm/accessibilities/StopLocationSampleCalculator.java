...
import com.pb.mtctm2.abm.application.SandagTripModeChoiceDMU;
        ...
        extends McLogsumsAppender
{
    ...
    private void runSampleProbabilitiesCalculator(ResourceBundle rb)
    {
        ...
        ChoiceModelApplication[] slcSoaModel = new ChoiceModelApplication[MAX_STOP_PURPOSE_INDEX+1];
        slcSoaModel[WORK_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, WORK_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[UNIV_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, UNIV_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[ESCORT_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, ESCORT_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[SHOP_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, SHOP_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[EAT_OUT_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, EAT_OUT_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[MAINT_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, MAINT_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[VISIT_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, VISIT_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
        slcSoaModel[DISCR_STOP_PURPOSE_INDEX] = new ChoiceModelApplication(slcSoaUecFile, DISCR_STOP_PURPOSE_INDEX, 0, rbMap, (VariableTable) slcSoaDmuObject);
         */
        ...
        double[] logsums = null;
        try {
            logsums = calculateTripModeChoiceLogsumForEstimationRecord( rbMap, mcModel[category], mcDmuObject, odtSet, mgra );
        }
        catch (Exception e){
            logger.error( "exception caught calculating trip mode choice logsum for survey record i =  " + (i + 1) );
        ...
        try {
            ...
            slcSoaSizeTerms = getSlcSoaSizeTermsForStopPurpose( stopPurpose, preSchoolInHh, gradeSchoolInHh, highSchoolInHh, sizeTerms );
        }
        catch (Exception e){
            logger.error( "exception caught calculating stop location choice size terms for survey record i =  " + (i + 1) );
            throw new RuntimeException();
        }
        ...
        try {
            ...
            int skimPeriodIndex = modelStructure.getSkimPeriodIndex( departPeriod ); // depart skim period
            double odDist = logsumHelper.getAnmSkimCalculator().getAutoSkims(origMgra, destMgra, skimPeriodIndex, false, logger)[AUTO_DIST_SKIM_INDEX];
            double osDist = logsumHelper.getAnmSkimCalculator().getAutoSkims(origMgra, mgra, skimPeriodIndex, false, logger)[AUTO_DIST_SKIM_INDEX];
            double sdDist = logsumHelper.getAnmSkimCalculator().getAutoSkims(mgra, destMgra, skimPeriodIndex, false, logger)[AUTO_DIST_SKIM_INDEX];
            ...
            double util = -999;
            if ( availability == 1 )
                util = Math.log( slcSoaSizeTerms[mgra] ) + absoulteDistanceDeviationCoefficient*distance;
            
            logUtilityCalculation( origMgra, mgra, destMgra, departPeriod, skimPeriodIndex,
                    osDist, sdDist, odDist, distance, slcSoaSizeTerms[mgra], util, logsums );           
        }
        catch (Exception e){
            logger.error( "exception caught calculating and logging utility calculation for survey record i =  " + (i + 1) );
            throw new RuntimeException();
        }
    ...
    private void logUtilityCalculation( int origMgra, int sampleMgra, int destMgra, int departPeriodIndex, int skimPeriodIndex,
            double osDist, double sdDist, double odDist, double distance, double size, double util, double[] logsums ) {
    ...
    private double[] getSlcSoaSizeTermsForStopPurpose ( int stopPurpose, boolean preSchoolInHh, boolean gradeSchoolInHh, boolean highSchoolInHh, double[][] sizeTerms ){
    ...
    private double[][] calculateSlcSoaSizeTerms( HashMap<String, String> rbMap, int sampleMgra ) {
        ...
        UtilityExpressionCalculator slcSoaSizeUec = new UtilityExpressionCalculator( new File(slcSoaSizeUecFile), slcSoaSizeUecModel, slcSoaSizeUecData, rbMap, null );
        ...
        for (int mgra : mgras)
        {
            ...
            if ( mgra == sampleMgra )
                slcSoaSizeUec.logAnswersArray(slcSoaLogger, "Stop Location SOA Size Terms, MGRA = " + mgra );
