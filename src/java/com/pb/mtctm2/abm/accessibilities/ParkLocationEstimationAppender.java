...
public final class ParkLocationEstimationAppender
{
    ...
    private static final int[] TYPE_INDICES = { OD_TYPE_INDEX, OP_TYPE_INDEX, PD_TYPE_INDEX };
    ...
    private void runAppender(ResourceBundle rb)
    {
        ...
        String outputFileName = Util.getStringValueFromPropertyMap( rbMap, OUTPUT_DATA_RECORDS_FILE_KEY );
        ...
        outStream.println( "seq,id,origMgra,destMgra,departPeriod,index,autoDist,autoTime,walkDist,walkTime" );
        ...
        for (int i = 0; i < estDataOdts.length; i++)
        {
            ...
            if( odtSet[ORIG] > 0 && odtSet[DEST] > 0 ) {
                ...
                int skimPeriodIndex = modelStructure.getSkimPeriodIndex( odtSet[DEPART] ); // depart skim period
                double[] autoSkims = anm.getAutoSkims( odtSet[ORIG], odtSet[DEST], skimPeriodIndex, (seq == DEBUG_EST_RECORD), logger );
            ...
            outStream.println( seq + "," + odtSet[ID] + "," + odtSet[ORIG] + "," + odtSet[DEST] + "," + odtSet[DEPART] + "," + TYPE_LABELS[odtSet[TYPE]] + "," + aDist + "," + aTime + "," + wDist + "," + wTime );

            if (seq % 1000 == 0)
                logger.info( "wrote PLC Estimation file record: " + seq );
    ...
    protected TableDataSet getEstimationDataTableDataSet(HashMap<String, String> rbMap)
    {
        ...
        if (estFileName == null)
        {
            ...
            logger.error("Properties file target: " + ESTIMATION_DATA_RECORDS_FILE_KEY + " not found.");
            logger.error("Please specify a filename value for the " + ESTIMATION_DATA_RECORDS_FILE_KEY + " property.");
        ...
        } catch (Exception e)
        {
            logger.fatal(String.format(
                "Exception occurred reading Sandag estimation data records file: %s into TableDataSet object.", estFileName));
            throw new RuntimeException(e);
        }
