...
//    private static final int      HIS_DEPART_PERIOD            = 325;
    ...
    private void runSkimsAppender(ResourceBundle rb)
    {
        ...
        String outputFileNameHis = Util.getStringValueFromPropertyMap(rbMap, "his.trip.skims.output.file");
        ...
        heading += getTransitSkimsHeaderRecord("wtw", wtw.getLocalSkimNames(), wtw.getPremiumSkimNames());
        heading += getTransitSkimsHeaderRecord("wtd", wtd.getLocalSkimNames(), wtd.getPremiumSkimNames());
        heading += getTransitSkimsHeaderRecord("dtw", dtw.getLocalSkimNames(), dtw.getPremiumSkimNames());
        ...
        {
            logger.fatal(String.format("Exception occurred opening output skims file: %s.", outputFileNameHis));
...
//        int[][] hisOdts = { { debugOrigMgra, debugDestMgra, departModelPeriod, 0 } };
        ...
        for (int i = 0; i < bestTapPairs.length; i++)
        {
            ...
            else
            {
                returnedSkims[i] = wtw.getWalkTransitWalkSkims(i,
                        BestTransitPathCalculator.findWalkTransitAccessTime(odt[0], bestTapPairs[i][0]),
                        BestTransitPathCalculator.findWalkTransitEgressTime(odt[1], bestTapPairs[i][1]),
        ...
        for (int i = 0; i < bestTapPairs.length; i++)
        {
            ...
            else
            {
                returnedSkims[i] = wtd.getWalkTransitDriveSkims(i, 
                        BestTransitPathCalculator.findWalkTransitAccessTime(odt[0], bestTapPairs[i][0]),
                        BestTransitPathCalculator.findDriveTransitEgressTime(odt[1], bestTapPairs[i][1]),
                        bestTapPairs[i][0], bestTapPairs[i][1], odt[2], loggingEnabled);
            }
        }
        ...
        for (int i = 0; i < bestTapPairs.length; i++)
        {
            ...
            else
            {
                returnedSkims[i] = dtw.getDriveTransitWalkSkims(i,
                        BestTransitPathCalculator.findDriveTransitAccessTime(odt[0], bestTapPairs[i][0]),
                        BestTransitPathCalculator.findWalkTransitEgressTime(odt[1], bestTapPairs[i][1]),
                        bestTapPairs[i][0], bestTapPairs[i][1], odt[2], loggingEnabled);
            }
        }
    ...
    private TableDataSet getHomeInterviewSurveyTableDataSet(HashMap<String, String> rbMap)
    {
        ...
        if (hisFileName == null)
        {
            ...
            logger.error("Please specify a filename value for the " + HIS_DATA_RECORDS_FILE_KEY + " property.");
            throw new RuntimeException();
        }
    ...
    public static void main(String[] args)
    {
        ...
        {
            System.out.println("please set values for command line arguments: properties file, orig mgra, dest mgra, depart model period.");
            System.exit(-1);
        }
        ...
        try {
            ...
            if ( serverAddress != null && serverPort > 0 ) {
                ...
                {
                    System.out.println("exception caught running ctramp model components -- exiting.");
                    e.printStackTrace();
                    throw new RuntimeException();
                }
            }
