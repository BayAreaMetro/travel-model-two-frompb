        ...
        implements Serializable
{
            ...
            BestTransitPathCalculator myBestPathUEC)
    {
        ...
        int dtwLocSkimMdPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.local.walk.md.page");
        int dtwLocSkimPmPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.local.walk.pm.page");
        int dtwLocSkimEvPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.local.walk.ev.page");
        
        int dtwPremSkimEaPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.premium.walk.ea.page");
        int dtwPremSkimAmPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.premium.walk.am.page");
        int dtwPremSkimMdPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.premium.walk.md.page");
        int dtwPremSkimPmPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.premium.walk.pm.page");
        int dtwPremSkimEvPage = Util.getIntegerValueFromPropertyMap(rbMap, "skim.drive.premium.walk.ev.page");

        File uecFile = new File(uecFileName);
        driveLocalWalkSkimUECs = new UtilityExpressionCalculator[NUM_PERIODS];
        driveLocalWalkSkimUECs[EA] = new UtilityExpressionCalculator(uecFile, dtwLocSkimEaPage, dataPage, rbMap, dmu);
        driveLocalWalkSkimUECs[AM] = new UtilityExpressionCalculator(uecFile, dtwLocSkimAmPage, dataPage, rbMap, dmu);
        driveLocalWalkSkimUECs[MD] = new UtilityExpressionCalculator(uecFile, dtwLocSkimMdPage, dataPage, rbMap, dmu);
        driveLocalWalkSkimUECs[PM] = new UtilityExpressionCalculator(uecFile, dtwLocSkimPmPage, dataPage, rbMap, dmu);
        driveLocalWalkSkimUECs[EV] = new UtilityExpressionCalculator(uecFile, dtwLocSkimEvPage, dataPage, rbMap, dmu);

        drivePremiumWalkSkimUECs = new UtilityExpressionCalculator[NUM_PERIODS];
        drivePremiumWalkSkimUECs[EA] = new UtilityExpressionCalculator(uecFile, dtwPremSkimEaPage, dataPage, rbMap, dmu);
        drivePremiumWalkSkimUECs[AM] = new UtilityExpressionCalculator(uecFile, dtwPremSkimAmPage, dataPage, rbMap, dmu);
        drivePremiumWalkSkimUECs[MD] = new UtilityExpressionCalculator(uecFile, dtwPremSkimMdPage, dataPage, rbMap, dmu);
        drivePremiumWalkSkimUECs[PM] = new UtilityExpressionCalculator(uecFile, dtwPremSkimPmPage, dataPage, rbMap, dmu);
        drivePremiumWalkSkimUECs[EV] = new UtilityExpressionCalculator(uecFile, dtwPremSkimEvPage, dataPage, rbMap, dmu);
    ...
    /**
     * Return the array of best drive-transit-walk tap pairs for the given origin
            ...
            Logger logger)
    {
        ...
        bestPathUEC.findBestDriveTransitWalkTaps(departPeriod, origMgra, destMgra, debug, primaryLogger);
        ...
        if (debug)
        {
            ...
            for (int i = 0; i < bestUtilities.length; i++)
            {
                ...
                logger.info(i + "," + mode[i] + "," + bestUtilities[i] +
                        "," + logValues[0] + "," + logValues[1] + "," + logValues[2] + "," + logValues[3] );
            }
     ...
     */
    public double[] getDriveTransitWalkSkims(int rideModeIndex, double pDriveTime, double aWalkTime, int origTap, int destTap,
            int departPeriod, boolean debug)
    {
        ...
        if (Modes.getIsPremiumTransit(rideModeIndex))
        {
            ...
            try {
                ...
                primaryLogger.error ("exception setting drive-transit-walk premium drive access time in stored array.", e);
            }
            ...
            try {
                ...
                primaryLogger.error ("exception setting drive-transit-walk premium walk egress time in stored array.", e);
            }
        ...
        } else
        {
            ...
            try {
                ...
                primaryLogger.error ("departPeriod=" + departPeriod + ", origTap=" + origTap + ", destTap=" + destTap + ", pDriveTime=" + pDriveTime);
                primaryLogger.error ("exception setting drive-transit-walk local drive access time in stored array.", e);
            }
            ...
            try {
                ...
                primaryLogger.error ("departPeriod=" + departPeriod + ", origTap=" + origTap + ", destTap=" + destTap + ", aWalkTime=" + aWalkTime);
                primaryLogger.error ("exception setting drive-transit-walk local walk egress time in stored array.", e);
            }
