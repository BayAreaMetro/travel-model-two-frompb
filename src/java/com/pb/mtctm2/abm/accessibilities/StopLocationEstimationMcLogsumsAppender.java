        ...
        extends McLogsumsAppender
{
    ...
    private void runLogsumAppender(ResourceBundle rb)
    {
        ...
        String outputFileName = Util.getStringValueFromPropertyMap(rbMap, "slc.est.skims.output.file");
    ...
    private void writeDcFile(HashMap<String, String> rbMap, PrintWriter outStream)
    {
        ...
        outStream.print("seq,portion,sampn,perno,tour_id,tripno,stopid,stopno,chosenMgra,chosenMgraLogsumIK,chosenMgraLogsumKJ");
        ...
        for (int i = 0; i < estDataOdts.length; i++)
        {
            ...
            outStream.print(seq + "," + odtSet[PORTION] + "," + odtSet[SAMPNO] + "," + odtSet[PERNO] + "," + odtSet[TOUR_ID] + "," + odtSet[TRIPNO] + "," + odtSet[STOPID] + "," + odtSet[STOPNO]);

            int category = PURPOSE_CATEGORIES[odtSet[TOUR_PURPOSE]];

            try {
                calculateTripModeChoiceLogsums(rbMap, mcModel[category], mcDmuObject, odtSet, mgraSet);
            }
            catch (Exception e){
            ...
            outStream.printf(",%.8f,%.8f", tripModeChoiceLogsums[0][0], tripModeChoiceLogsums[0][1]);

            // write logsum sets for each dest in the sample to file 1
            for (int m = 1; m < tripModeChoiceLogsums.length; m++)
            {
                outStream.print("," + mgraSet[m - 1]);
                outStream.printf(",%.8f,%.8f", tripModeChoiceLogsums[m][0], tripModeChoiceLogsums[m][1]);
            }
            outStream.print("\n");
    ...
    public static void main(String[] args)
    {
        ...
        StopLocationEstimationMcLogsumsAppender appender = new StopLocationEstimationMcLogsumsAppender( rbMap );
