...
public class ReportBuilder {
    ...
    public static void main(String ... args) {
        ResourceBundle properties =  ResourceUtil.getResourceBundle("serpm_reports"); //SERPM reports resource bundle comes from properties file
        ...
        //Force schema names to start with letters
        if(!Character.isLetter(schema.charAt(0)))
        	schema = "S_" + schema;
        ...
        if(tables.length != worksheets.length)
        	throw new RuntimeException("The number of SQL tables and declared worsheets must be equal");
        ...
        String databaseServerInstance = properties.containsKey("database.instance") ? properties.getString("database.instance") : null;

        Map<String,String> detokenizingMap = new HashMap<String,String>();
        detokenizingMap.put(schemaKey,schema);

        ReportBuilder rb = new ReportBuilder(ipaddress,port,databaseName,username,password,databaseServerInstance);
        if(args.length > 0 && args[0].length() > 0)
        	rb.buildSQLDatabase(args[0]);
    ...
    public ReportBuilder(String ipAddress, int port, String databaseName, String username, String password, String databaseServerInstance) {
        connectionUrl = formConnectionUrl(ipAddress,port,databaseName,username,password,databaseServerInstance);
    }

    public void buildReport(List<String> sqlFiles, Map<String,String> detokenizingMappings, String schema, Map<String,String> tableToWorksheetMapping, String workbookFile) {
    	executeSqlFiles(sqlFiles,detokenizingMappings);
        writeTablesToExcel(schema,tableToWorksheetMapping,workbookFile);
    }
    
    private void buildSQLDatabase(String sql_table_import) {
        logger.info("Building " + sql_table_import);
        executeSql(readFile(sql_table_import));
    }
    ...
    private void writeTablesToExcel(String schema, Map<String,String> tableToWorksheetMapping, String workbookFile) {
    ...
    private void writeResultSetToExcel(ResultSet resultSet, Workbook workbook, String worksheet) throws SQLException {
    ...
    private void writeValue(Cell cell, Object object) {
        ...
        private String formConnectionUrl(String ipAddress, int port, String databaseName, String username, String password, String instance) {
            String url = "jdbc:jtds:sqlserver://" + ipAddress + ":" + port + "/" + databaseName;
            if (username != null)
                url += ";user=" + username + ";password=" + password; //not super secure, but ok for now - probably will use SSO normally
            if (instance != null)
