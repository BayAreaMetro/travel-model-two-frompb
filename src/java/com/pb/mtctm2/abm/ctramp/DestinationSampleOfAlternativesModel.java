        ...
        implements Serializable
{
    ...
    private transient Logger         logger            = Logger
                                                               .getLogger(DestinationSampleOfAlternativesModel.class);
            ...
            double[][] dcSizeArray, DcSoaDMU dcSoaDmuObject, int[] soaUecIndices)
    {
        ...
        // create an array of sample of alternative ChoiceModelApplication objects
        // for each purpose
        setupSampleOfAlternativesChoiceModelArrays(propertyMap, dcSizeArray, dcSoaDmuObject, soaUecIndices);
            ...
            double[][] dcSizeArray, DcSoaDMU dcSoaDmuObject, int[] soaUecIndices)
    {
        ...
        for ( int uecIndex : soaUecIndices ) {
            // if the uec sheet for the size segment is not in the map, add it, otherwise, get it from the map
        ...
        // the value of soaModelIndex is the number of ChoiceModelApplication objects to create
        ...
        for (int uecIndex : modelIndexMap.keySet() )
        {
            ...
            {
                logger.error( String.format("exception caught setting up DC SOA ChoiceModelApplication[%d] for modelIndex=%d of %d models", i, modelIndex, modelIndexMap.size()) );
     ...
     * It would be called subsequently if a shadow pricing methodology is applied to
     * reset the scaled size terms and corresponding availabilities and sample
     * arrays.
     */
    public void setAvailabilityForSampleOfAlternatives(double[][] dcSizeArray)
    {
        ...
        // declare dimensions for the alternative availability array by purpose and
        ...
        {
        	for (int j = 0; j < mgraManager.getMgras().size(); j++)
            {
                int k = mgraManager.getMgras().get(j);
        		...
        		escortAvailable[k] = true;
                escortSample[k] = 1;
            }
        }

        numberOfSoaChoiceAlternatives = mgraManager.getMgras().size();
    }
            ...
            Person person, String segmentName, int segmentIndex, int origMgra)
    {
        ...
        if ( tour != null && tour.getTourCategory() == ModelStructure.AT_WORK_CATEGORY ) {
            ...
            if ( ALWAYS_COMPUTE_PROBABILITIES || origMgra != currentWorkMgra ) {
                ...
                // clear the probabilities stored for the current origin mgra, for each DC segment
            ...
            // If the sample of alternatives choice probabilities have not been computed for the current origin mgra
            // and segment specified, compute them.
            if ( subtourProbabilitiesCache[segmentIndex] == null ) {
                computeSampleOfAlternativesChoiceProbabilities( dcSoaDmuObject, tour, person, segmentName, segmentIndex, origMgra);
                soaProbabilitiesCalculationCount++;
            }

        }
        else if ( tour != null &&  tour.getTourPrimaryPurpose().equalsIgnoreCase( ModelStructure.ESCORT_PRIMARY_PURPOSE_NAME ) ) {

            // always update probabilities for Escort tours
            // clear the probabilities stored for the current origin mgra, for the Escort DC segment 
        ...
        // select sampleSize alternatives based on probabilitiesList[origTaz], and
        ...
        for (int i = 0; i < sampleSize; i++)
        {
            ...
            int chosenAlt = -1;
            if ( tour != null &&  tour.getTourCategory() == ModelStructure.AT_WORK_CATEGORY )
                chosenAlt = Util.binarySearchDouble( subtourCumProbabilitiesCache[segmentIndex], rn ) + 1;
            else
                chosenAlt = Util.binarySearchDouble( cumProbabilitiesCache[segmentIndex], rn ) + 1;

            // write choice model alternative info to log file
            if (hhObj.getDebugChoiceModels())
            {
                choiceModel[modelIndex].logSelectionInfo(
                    String.format("Sample Of Alternatives Choice for segmentName=%s, segmentIndex=%d, modelIndex=%d, origMgra=%d", segmentName, segmentIndex, modelIndex, origMgra),
                    String.format("HHID=%d, rn=%.8f, rnCount=%d", hhObj.getHhId(), rn, (rnCount + i)), rn, chosenAlt
                    );
            }
        ...
        // sampleSize random number draws were made from the Random object for the
        // current household,
        // so update the count in the hh's Random.
        hhObj.setHhRandomCount(rnCount);


        // create arrays of the unique chosen alternatives and the frequency with
        ...
        // loop through these arrays, construct final sample[] and corrections[].
        ...
        for (k = 0; k < numUniqueAlts; k++)
        {
            ...
            double prob = 0;
            if ( tour != null &&  tour.getTourCategory() == ModelStructure.AT_WORK_CATEGORY )
                prob = subtourProbabilitiesCache[segmentIndex][alt-1];
            else
                prob = probabilitiesCache[segmentIndex][alt-1];
     ...
     * encountered for the purpose and origin taz. Once the sample of alternatives
     * choice probabilities for a purpose and origin taz are computed, they are
     * stored in an array and used by other decision makers with the same purpose and
     * origin taz.
     * 
     * @param probabilitiesList is the probabilities array for the given purpose in
     *            which choice probabilities will be saved for the origin of the
     *            tour/place to be selected.
     * @param cumProbabilitiesList is the probabilities array for the given purpose
     *            in which choice cumulative probabilities will be saved for the
     *            origin of the tour/place to be selected.
     * @param choiceModel the ChoiceModelApplication object for the purpose
     * @param tour the tour object for whic destination choice is required, or null
     *            if a usual work/school location is being chosen
     * @param person the person object for whom the choice is being made
     * @param segmentName the name of the segment the choice is being made for - for
     *            logging
     * @param segmentindex the index associated with the segment
     * @param origMgra the index associated with the segment
     * @param distances array frpm origin MGRA to all MGRAs
     */
            ...
            Tour tour, Person person, String segmentName, int segmentIndex, int origMgra)
    {
        ...
        // prepare a trace log header that the choiceModel object will write prior to
        // UEC trace logging
        ...
        // information,
        // create a trace logger header and write prior to the choiceModel computing
        // utilities
        if (hhObj.getDebugChoiceModels())
        {
            ...
            if (tour == null)
            {
                ...
                choiceModelDescription = String.format("Usual Location Sample of Alternatives Choice Model for: Segment=%s", segmentName);
            ...
            {
                choiceModelDescription = String.format("Destination Choice Model for: Segment=%s, TourId=%d", segmentName, tour.getTourId());
                decisionMakerLabel = String.format("HH=%d, PersonNum=%d, PersonType=%s", person.getHouseholdObject().getHhId(), person.getPersonNum(), person.getPersonType());
            }

            // log headers to traceLogger if the person making the choice is from a
            // household requesting trace information
            choiceModel[choiceModelIndex].choiceModelUtilityTraceLoggerHeading(choiceModelDescription, decisionMakerLabel);

        }

        try {
            choiceModel[choiceModelIndex].computeUtilities(dcSoaDmuObject, dcSoaDmuObject.getDmuIndexValues(),
                destsAvailable[segmentIndex], destsSample[segmentIndex]);
        }
        catch (Exception e){
        ...
        if ( choiceModel[choiceModelIndex].getAvailabilityCount() == 0 ){
            ...
            choiceModel[choiceModelIndex].logUECResultsSpecificAlts(dcSoaLogger, "debugging", debugAlts);
        }
        ...
        if ( tour != null &&  tour.getTourCategory() == ModelStructure.AT_WORK_CATEGORY ) {
            ...
            subtourCumProbabilitiesCache[segmentIndex] = Arrays.copyOf( choiceModel[choiceModelIndex].getCumulativeProbabilities(), choiceModel[choiceModelIndex].getNumberOfAlternatives() );
            subtourProbabilitiesCache[segmentIndex] = Arrays.copyOf( choiceModel[choiceModelIndex].getProbabilities(), choiceModel[choiceModelIndex].getNumberOfAlternatives() );
        }
        else {

            cumProbabilitiesCache[segmentIndex] = Arrays.copyOf( choiceModel[choiceModelIndex].getCumulativeProbabilities(), choiceModel[choiceModelIndex].getNumberOfAlternatives() );
            probabilitiesCache[segmentIndex] = Arrays.copyOf( choiceModel[choiceModelIndex].getProbabilities(), choiceModel[choiceModelIndex].getNumberOfAlternatives() );
            
            if ( hhObj.getDebugChoiceModels() ){
                PrintWriter out = null;
                try {
                    out = new PrintWriter( new BufferedWriter( new FileWriter( new File( "soaProbs.csv" ) ) ) );
        ...
        if (hhObj.getDebugChoiceModels() && ALLOW_DEBUG )
        {
            //if ( dcSoaLogger.isDebugEnabled() ){
                ...
                choiceModel[choiceModelIndex].logUECResultsSpecificAlts(dcSoaLogger, choiceModelDescription + ", "
                        + decisionMakerLabel, altsToLog);
                ...
                for ( int i=1; i < altsToLog.length; i++ )
                    dcSoaLogger.info( "alt=" + (altsToLog[i]+1) + ", util=" + utils[altsToLog[i]] + ", prob=" + probs[altsToLog[i]] );
