...
/**
 * This class will be used for determining the trip departure time for outbound stops, trip arrival time
 * for inbound stops, location of stops, and trip mode for trips between stops
 * on individual mandatory, individual non-mandatory and joint tours.
 *
 * @author Jim Hicks
 * @version Oct 2010
 */
public class IntermediateStopChoiceModels implements Serializable {
    ...
    private static int       	   MAX_PLC_SAMPLE_SIZE;
    ...
    public static final int[]   SLC_SIZE_SEGMENT_INDICES = {
        WORK_STOP_PURPOSE_SOA_SIZE_INDEX,
        UNIV_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_0_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_PS_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_GS_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_HS_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_PS_GS_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_PS_HS_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_GS_HS_STOP_PURPOSE_SOA_SIZE_INDEX,
        ESCORT_PS_GS_HS_STOP_PURPOSE_SOA_SIZE_INDEX,
        SHOP_STOP_PURPOSE_SOA_SIZE_INDEX,
        MAINT_STOP_PURPOSE_SOA_SIZE_INDEX,
        EAT_OUT_STOP_PURPOSE_SOA_SIZE_INDEX,
    ...
    private int availAltsToLog = 55; // set larger than number of mode alts to suppress this logging
//    private int availAltsToLog = 5;


    // set this constant for checking the number of times depart/arrive period selection is made so that no infinite loop occurs.
     ...
     */
    public IntermediateStopChoiceModels( HashMap<String, String> propertyMap, ModelStructure myModelStructure, CtrampDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper ) {
    ...
    private void setupStopLocationChoiceModels( HashMap<String, String> propertyMap, CtrampDmuFactoryIf dmuFactory  ) {
        ...
        slcSoaModel[OTHER_STOP_LOC_SOA_SHEET_INDEX] = new ChoiceModelApplication(slcSoaUecFile, OTHER_STOP_LOC_SOA_SHEET_INDEX, 0, propertyMap, (VariableTable)stopLocDmuObj);
        slcSoaModel[WALK_STOP_LOC_SOA_SHEET_INDEX] = new ChoiceModelApplication(slcSoaUecFile, WALK_STOP_LOC_SOA_SHEET_INDEX, 0, propertyMap, (VariableTable)stopLocDmuObj);
        slcSoaModel[BIKE_STOP_LOC_SOA_SHEET_INDEX] = new ChoiceModelApplication(slcSoaUecFile, BIKE_STOP_LOC_SOA_SHEET_INDEX, 0, propertyMap, (VariableTable)stopLocDmuObj);

        
        int numSlcSoaAlternatives = slcSoaModel[OTHER_STOP_LOC_SOA_SHEET_INDEX].getNumberOfAlternatives();
        stopLocDmuObj = dmuFactory.getStopLocationDMU();
        ...
        // set the second argument to a positive, non-zero mgra value to get logging for the size term calculation for the specified mgra.
        ...
        mcModelArray[WORK_CATEGORY] = new ChoiceModelApplication(mcUecFile, WORK_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
        mcModelArray[UNIVERSITY_CATEGORY] = new ChoiceModelApplication(mcUecFile, UNIVERSITY_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
        mcModelArray[SCHOOL_CATEGORY] = new ChoiceModelApplication(mcUecFile, SCHOOL_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
        mcModelArray[MAINTENANCE_CATEGORY] = new ChoiceModelApplication(mcUecFile, MAINTENANCE_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
        mcModelArray[DISCRETIONARY_CATEGORY] = new ChoiceModelApplication(mcUecFile, DISCRETIONARY_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
        mcModelArray[SUBTOUR_CATEGORY] = new ChoiceModelApplication(mcUecFile, SUBTOUR_SHEET, 0, propertyMap, (VariableTable) mcDmuObject);
        ...
        int slcMandModelPage = Integer.parseInt( propertyMap.get(PROPERTIES_UEC_MAND_SLC_MODEL_PAGE) );
        int slcMaintModelPage = Integer.parseInt( propertyMap.get(PROPERTIES_UEC_MAINT_SLC_MODEL_PAGE) );
        int slcDiscrModelPage = Integer.parseInt( propertyMap.get(PROPERTIES_UEC_DISCR_SLC_MODEL_PAGE) );
        slcModelArray = new ChoiceModelApplication[3];
        slcModelArray[MAND_SLC_MODEL_INDEX] = new ChoiceModelApplication( slcUecFile, slcMandModelPage, slcDataPage, propertyMap, (VariableTable)stopLocDmuObj );
        slcModelArray[MAINT_SLC_MODEL_INDEX] = new ChoiceModelApplication( slcUecFile, slcMaintModelPage, slcDataPage, propertyMap, (VariableTable)stopLocDmuObj );
        slcModelArray[DISCR_SLC_MODEL_INDEX] = new ChoiceModelApplication( slcUecFile, slcDiscrModelPage, slcDataPage, propertyMap, (VariableTable)stopLocDmuObj );
        ...
        // declare the arrays for storing stop location choice ik and kj segment mode choice probability arrays
        ...
        // declare the arrays that holds ik and kj segment logsum values for each location choice sample alternative
        tripModeChoiceLogsums = new double[sampleSize+1];
        
        // declare the arrays that holds ik and kj distance values for each location choice sample alternative
        // these are set as stop location dmu attributes for stop orig to stop alt, and stop alt to half-tour final dest.
        ikDistance = new double[sampleSize+1];
        kjDistance = new double[sampleSize+1];

        // declare the arrays that holds ik and kj distance values for each location choice sample alternative
        // these are set as stop location dmu attributes for tour orig to stop alt, and stop alt to tour dest.
        okDistance = new double[sampleSize+1];
        kdDistance = new double[sampleSize+1];
        
        // this array has elements with values of 0 if utility is not to be computed for alternative, or 1 if utility is to be computed.
        inSample = new int[sampleSize+1];

        // this array has elements that are boolean that set availability of alternative - unavailable altrnatives do not get utility computed.
        ...
        // create the array of 1s for MGRAs that have a non-empty set of TAPs within walk egress distance of them
        // for the setting walk transit available for teh stop location alternatives.
    ...
    private void setupParkingLocationModel( HashMap<String, String> propertyMap, CtrampDmuFactoryIf dmuFactory ) {
        ...
        plcModel =  new ChoiceModelApplication( plcUecFile, plcModelPage, plcDataPage, propertyMap, (VariableTable)parkingChoiceDmuObj );

        //get number of max plc samples from alts file
        MAX_PLC_SAMPLE_SIZE = plcModel.getNumberOfAlternatives();
    ...
    private double[][] calculateLnSlcSizeTerms( HashMap<String, String> rbMap, int logMgra ) {
        ...
        UtilityExpressionCalculator slcSizeUec = new UtilityExpressionCalculator( new File(slcSizeUecFile), slcSizeUecModel, slcSizeUecData, rbMap, null );        
        ...
        // create the array for storing logged size term values - to be returned by this method
        ...
        for (int mgra : mgras)
        {
            ...
            // if a logMgra values > 0 was specified, log the size term utility calculation for that mgra
            if ( mgra == logMgra )
                slcSizeUec.logAnswersArray(slcSoaLogger, "Stop Location SOA Size Terms, MGRA = " + mgra );
    ...
    private double[] getLnSlcSizeTermsForStopPurpose ( int stopPurpose, Household hh ){
        ...
        // save backup arrays with original availability and sample values.
        // the procedure to generate availability for transit tours overwrites the arrays used by the UECs,
        // so they need to be restored after that happens.
        soaSampleBackup = new int[soaSample.length];
        soaAvailabilityBackup = new boolean[soaSample.length];
    ...
    private double[] getLnSlcSizeTermsForEscortStopPurpose ( Household hh ){
        ...
        // set booleans for presence of preschool, grade school, and high school students in the hh
        ...
        // if hh has no preschool, grade school or high school children, set the array to that specific size term field
        ...
        // if hh has a preschool child and no gs or hs, set the array to that specific size term field
        ...
        // if hh has a grade school child and no ps or hs, set the array to that specific size term field
        else if ( !psInHh && gsInHh && !hsInHh ){
        ...
        // if hh has a high school child and no ps or gs, set the array to that specific size term field
        else if ( !psInHh && !gsInHh && hsInHh ){
        ...
        // if hh has a preschool and a grade school child and no hs, set the array to that specific size term field
        else if ( psInHh && gsInHh && !hsInHh ) {
        ...
        // if hh has a preschool and a high school child and no gs, set the array to that specific size term field
        else if ( psInHh && !gsInHh && hsInHh ){
        ...
        // if hh has a grade school and a high school child and no ps, set the array to that specific size term field
        else if ( !psInHh && gsInHh && hsInHh ){
        ...
        // if hh has a preschool a grade school and a high school child, set the array to that specific size term field
        else if ( psInHh && gsInHh && hsInHh ){
    ...
    public void applyModel( Household household, boolean withTiming ) {
        ...
        for(int j=1;j<personArray.length;++j){
            ...
            for ( Tour tour : tours ) {
                ...
                if ( withTiming )
                    applyForOutboundStopsWithTiming( tour, person, household );
                else
                    applyForOutboundStops( tour, person, household );
                
                if ( withTiming )
                    applyForInboundStopsWithTiming( tour, person, household );
                else
        ...
        if ( jointTours != null ) {
            ...
            for ( Tour tour : jointTours ) {
                ...
                if ( withTiming )
                    applyForOutboundStopsWithTiming( tour, null, household );
                else
                    applyForOutboundStops( tour, null, household );
                
                if ( withTiming )
                    applyForInboundStopsWithTiming( tour, null, household );
                else
    ...
    private void applyForInboundStops( Tour tour, Person person, Household household ) {
        ...
        if ( stops != null) {
            ...
            // get the outbound stops array - note, if there were no outbound stops for half-tour, one stop object would have been generated
            // to hold information about the half-tour trip, so this array should never be null.
            ...
            if ( obStops == null ) {
                ...
                logger.error( "HHID=" + household.getHhId() + ", persNum=" + person.getPersonNum() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() + ", tourMode=" + tour.getTourModeChoice() );
    ...
    private void applySlcModel ( Household household, Person person, Tour tour, Stop[] stops, int origMgra, int destMgra, boolean directionIsInbound ) {
        ...
        anm.getDistancesFromMgra( origMgra, tourOrigToAllMgraDistances, modelStructure.getTourModeIsSovOrHov( tour.getTourModeChoice() ) );
        anm.getDistancesFromMgra( destMgra, tourDestToAllMgraDistances, modelStructure.getTourModeIsSovOrHov( tour.getTourModeChoice() ) );
        //anm.getDistancesToMgra( destMgra, tourDestToAllMgraDistances, modelStructure.getTourModeIsSovOrHov( tour.getTourModeChoice() ) );
        ...
        if (stops != null) {
            ...
            for ( int i=0; i < stops.length; i++ ) {
                ...
                // if i is 0, the stop origin is set to origMgra; otherwise stop orig is the chosen dest from the previous stop.
                if ( i == 0 )
                    newOrig = origMgra;
                else
                    newOrig = lastDest;                
                ...
                stopLocDmuObj.setDmuIndexValues( household.getHhId(), household.getHhMgra(), newOrig, destMgra );

                int choice = -1;
                int selectedIndex = -1;
                int modeAlt = -1;
                // if not the last stop object, make a destination choice and a mode choice from IK MC probabilities;
                // otherwise stop dest is set to destMgra, and make a mode choice from KJ MC probabilities.
                if ( i < stops.length - 1 ) {
                    ...
                    try {
                        ...
                        if ( household.getDebugChoiceModels() ){
                            smcLogger.info( "Monte Carlo selection for determining Mode Choice from IK Probabilities for " + (stop.isInboundStop() ? "INBOUND" : "OUTBOUND") + " stop." );
                            ...
                            smcLogger.info( "StopID=" + (stop.getStopId()+1) + " of " + (stops.length - 1) + " stops, inbound=" + stop.isInboundStop() + ", stopPurpose=" + stop.getDestPurpose() + ", stopDepart=" + stop.getStopPeriod() + ", stopOrig=" + stop.getOrig() + ", stopDest=" + stop.getDest() );
                        }

                        modeAlt = selectModeFromProbabilities( stop, mcCumProbsSegmentIk[selectedIndex] );
                        if ( modeAlt < 0 ) {
                            logger.info( "error getting trip mode choice for IK proportions, i=" + i );
                            logger.info( "HHID=" + household.getHhId() + ", persNum=" + person.getPersonNum() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() + ", tourMode=" + tour.getTourModeChoice() );
                            logger.info( "StopID=" + (stop.getStopId()+1) + " of " + (stops.length - 1) + " stops, inbound=" + stop.isInboundStop() + ", stopPurpose=" + stop.getDestPurpose() + ", stopDepart=" + stop.getStopPeriod() + ", stopOrig=" + stop.getOrig() + ", stopDest=" + stop.getDest() );
                            throw new RuntimeException();
                        }
                        ...
                        logger.error ( String.format( "Exception caught processing %s stop location choice model for %s type tour %s stop:  HHID=%d, personNum=%s, stop=%d.", ( stopLocDmuObj.getInboundStop() == 1 ? "inbound" : "outbound"), tour.getTourCategory(), tour.getTourPurpose(), household.getHhId(), ( person == null ? "N/A" : Integer.toString(person.getPersonNum()) ), (i+1) ) );
                    ...
                    // last stop on half-tour, so dest is the half-tour final dest, and oldSelectedIndex was
                    ...
                    modeAlt = selectModeFromProbabilities( stop, mcCumProbsSegmentKj[oldSelectedIndex] );
                    if ( modeAlt < 0 ) {
                        logger.error( "error getting trip mode choice for KJ proportions, i=" + i );
                        logger.error( "HHID=" + household.getHhId() + ", persNum=" + person.getPersonNum() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() + ", tourMode=" + tour.getTourModeChoice() );
                        logger.error( "StopID=End of " + (stop.isInboundStop() ? "INBOUND" : "OUTBOUND") + " half-tour, stopPurpose=" + stop.getDestPurpose() + ", stopDepart=" + stop.getStopPeriod() + ", stopOrig=" + stop.getOrig() + ", stopDest=" + stop.getDest()  );
                        throw new RuntimeException();
                    }

                    // last stop on tour, so if inbound, only need park location choice if tour is work-based subtour;
        ...
        // create a stop object to hold attributes for orig, dest, mode, departtime, etc.
        // for the half-tour with no stops.
            ...
            if ( !directionIsInbound ) {
                origStopPurpose = tour.getTourCategory().equalsIgnoreCase(ModelStructure.AT_WORK_CATEGORY) ? "Work" : "Home";
                destStopPurpose = tour.getTourPrimaryPurpose();
            }
            else {
                origStopPurpose = tour.getTourPrimaryPurpose();
                destStopPurpose = tour.getTourCategory().equalsIgnoreCase(ModelStructure.AT_WORK_CATEGORY) ? "Work" : "Home";
            }

            Stop stop = null;
            try {
                stop = tour.createStop( modelStructure, origStopPurpose, destStopPurpose, directionIsInbound, tour.getTourCategory().equalsIgnoreCase(ModelStructure.AT_WORK_CATEGORY) );            
            }
            catch ( Exception e ){
            ...
            // set stop origin and destination mgra, the stop period based on direction, then calculate the half-tour trip mode choice
            ...
            if ( modeAlt < 0 ) {
                ...
                logger.info( "HHID=" + household.getHhId() + ", persNum=" + person.getPersonNum() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() );
                logger.info( "StopID=" + (stop.getStopId()+1) + " of no stops, inbound=" + stop.isInboundStop() + ", stopPurpose=" + stop.getDestPurpose() + ", stopDepart=" + stop.getStopPeriod() + ", stopOrig=" + stop.getOrig() + ", stopDest=" + stop.getDest() );
                throw new RuntimeException();
            }
    ...
    private void applySlcModelWithTiming ( Household household, Person person, Tour tour, Stop[] stops, int origMgra, int destMgra, boolean directionIsInbound ) {
                    ...
                    try {
                        ...
                        if ( household.getDebugChoiceModels() ){
                            if ( tour.getTourCategory().equalsIgnoreCase( ModelStructure.JOINT_NON_MANDATORY_CATEGORY ) ) {
                                smcLogger.info( "Monte Carlo selection for determining Mode Choice from IK Probabilities for " + (stop.isInboundStop() ? "INBOUND" : "OUTBOUND") + " for joint tour stop." );
                        ...
                        logger.error ( String.format( "Exception caught processing %s stop location choice model.", ( stopLocDmuObj.getInboundStop() == 1 ? "inbound" : "outbound") ) );
                        logger.error ( "HHID=" + household.getHhId() + ", persNum=" + person.getPersonNum() + ", tour category=" + tour.getTourCategory() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() + ", tourMode=" + tour.getTourModeChoice() );
                        logger.error ( "StopID=" + (stop.getStopId()+1) + " of " + (stops.length - 1) + " stops, inbound=" + stop.isInboundStop() + ", stopPurpose=" + stop.getDestPurpose() + ", stopDepart=" + stop.getStopPeriod() + ", stopOrig=" + stop.getOrig() + ", stopDest=" + stop.getDest() );
                        logger.error ( String.format( "origMgra=%d, destMgra=%d, newOrig=%d, lastDest=%d, modeAlt=%d, selectedIndex=%d, choice=%d.", origMgra, destMgra, newOrig, lastDest, modeAlt, selectedIndex, choice ) );
                        throw new RuntimeException(e);
                    }
        ...
        // for the half-tour with no stops.
            ...
            if ( modeAlt < 0 ) {
                logger.info( "error getting trip mode choice for half-tour with no stops." );
                logger.info( "HHID=" + household.getHhId() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() );
                logger.info( "StopID=" + (stop.getStopId()+1) + " of no stops, inbound=" + stop.isInboundStop() + ", stopPurpose=" + stop.getDestPurpose() + ", stopDepart=" + stop.getStopPeriod() + ", stopOrig=" + stop.getOrig() + ", stopDest=" + stop.getDest() );
    ...
    private int selectDestination(Stop s) {
        ...
        //int[] debugLoggingSample = { 0, 4886, 16859, 18355, 3222, 14879, 26894, 16512, 9908, 18287, 14989 };
        ...
        if ( household.getDebugChoiceModels() ) {
        	household.logHouseholdObject( "Pre Stop Location Choice for trip: HH_" + household.getHhId() + ", Pers_" + tour.getPersonObject().getPersonNum() + ", Tour Purpose_" + tour.getTourPurpose() + ", Tour_" + tour.getTourId() + ", Tour Purpose_" + tour.getTourPurpose() + ", Stop_" + (s.getStopId()+1), modelLogger );
            ...
            household.logStopObject("Pre Stop Location Choice for stop " + (s.getStopId()+1), modelLogger, s, modelStructure );
        ...
        stopLocDmuObj.setLogSize( getLnSlcSizeTermsForStopPurpose(s.getStopPurposeIndex(), household) );
        
        // get the array of distances from the stop origin mgra to all MGRAs and set in the dmu object
        anm.getDistancesFromMgra( s.getOrig(), distanceFromStopOrigToAllMgras, modelStructure.getTourModeIsSovOrHov( tour.getTourModeChoice() ) );
        ...
        // get the array of distances to the half-tour final destination mgra from all MGRAs and set in the dmu object
        if ( s.isInboundStop() ){
            // if inbound, final half-tour destination is the tour origin
            anm.getDistancesToMgra( tour.getTourOrigMgra(), distanceToFinalDestFromAllMgras, modelStructure.getTourModeIsSovOrHov( tour.getTourModeChoice() ) );
            stopLocDmuObj.setDistancesToDestMgra( distanceToFinalDestFromAllMgras );
        
            // set the distance from the stop origin to the final half-tour destination
            stopLocDmuObj.setOrigDestDistance( distanceFromStopOrigToAllMgras[tour.getTourOrigMgra()] );
        }
        else{
            // if outbound, final half-tour destination is the tour destination
            anm.getDistancesToMgra( tour.getTourDestMgra(), distanceToFinalDestFromAllMgras, modelStructure.getTourModeIsSovOrHov( tour.getTourModeChoice() ) );
            stopLocDmuObj.setDistancesToDestMgra( distanceToFinalDestFromAllMgras );
        
            // set the distance from the stop origin to the final half-tour destination
            stopLocDmuObj.setOrigDestDistance( distanceFromStopOrigToAllMgras[tour.getTourDestMgra()]);
        }
        ...
        if ( household.getDebugChoiceModels() ) {
            ...
            decisionMakerLabel = String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopId=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourPurpose(), tour.getTourModeChoice(), tour.getTourId(), s.getDestPurpose(), (s.getStopId()+1) );
        ...
        int slcModelIndex = -1;
        if ( tour.getTourCategory().equalsIgnoreCase( ModelStructure.MANDATORY_CATEGORY ) )
            slcModelIndex = MAND_SLC_MODEL_INDEX;
                ...
                || tour.getTourPrimaryPurposeIndex() == ModelStructure.OTH_MAINT_PRIMARY_PURPOSE_INDEX )
            slcModelIndex = MAINT_SLC_MODEL_INDEX;
        else
            slcModelIndex = DISCR_SLC_MODEL_INDEX;
        
        
        slcModelArray[slcModelIndex].computeUtilities(stopLocDmuObj, stopLocDmuObj.getDmuIndexValues(), sampleAvailability, inSample);
        ...
        if ( household.getDebugChoiceModels() || chosen < 0 ) {
            ...
            if ( chosen < 0 ) {
                ...
                modelLogger.error( "setting debug to true and recomputing sample of alternatives selection." );
                modelLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopId=%d, StopOrig=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourPurpose(), tour.getTourModeChoice(), tour.getTourId(), s.getDestPurpose(), (s.getStopId()+1), s.getOrig() ) );

                choiceModelDescription = "Stop Location Choice";
                decisionMakerLabel = String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopId=%d, StopOrig=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourPurpose(), tour.getTourModeChoice(), tour.getTourId(), s.getDestPurpose(), (s.getStopId()+1), s.getOrig() );
                loggingHeader = String.format( "%s for %s", choiceModelDescription, decisionMakerLabel );
                ...
                modelLogger.error("Person num: " + personNum  + ", Person type: " + personTypeString );
                modelLogger.error("Alternative             Availability           Utility       Probability           CumProb");
                modelLogger.error("---------------------   ------------       -----------    --------------    --------------");
                ...
                for(int j=1; j <= numAltsInSample; j++){
                    ...
                    modelLogger.error(String.format("%-21s%15s%18.6e%18.6e%18.6e", altString, availabilities[j], utilities[j-1], probabilities[j-1], cumProb));
                }

                modelLogger.error(" ");
                String altString = String.format( "%-3d %5d", selectedIndex, -1 );
                modelLogger.error( String.format("Choice: %s, with rn=%.8f, randomCount=%d", altString, rn, randomCount ) );
                ...
                slcModelArray[slcModelIndex].logAlternativesInfo ( choiceModelDescription, decisionMakerLabel );
                slcModelArray[slcModelIndex].logSelectionInfo ( choiceModelDescription, decisionMakerLabel, rn, chosen );
                ...
                logger.error (String.format( "Error for HHID=%d, PersonNum=%d, no available %s stop destination choice alternatives to choose from in choiceModelApplication.", tour.getHhId(), tour.getPersonObject().getPersonNum(), tour.getTourPurpose()));
                throw new RuntimeException();
            ...
            for(int j=1; j <= numAltsInSample; j++){
                ...
                cumProb += probabilities[j-1];
                String altString = String.format( "%-3d %5d", j, alt );
                modelLogger.info(String.format("%-21s%15s%18.6e%18.6e%18.6e", altString, availabilities[j], utilities[j-1], probabilities[j-1], cumProb));
            }
    ...
    private void setupStopLocationChoiceAlternativeArrays( int numAltsInSample, Stop s ) {
        ...
        // create arrays for ik and kj mode choice logsums for the stop origin, the sample stop location, and the half-tour final destination.
        ...
        int halfTourFinalDest = s.isInboundStop() ? s.getTour().getTourOrigMgra() : s.getTour().getTourDestMgra();
        ...
        for (int i=1; i <= numAltsInSample; i++) {
            ...
            // set distances from tour anchor points to stop location alternative.
            ...
            mcDmuObject.setATazTerminalTime(tazs.getDestinationTazTerminalTime(mgraManager.getTaz(altMgra)));

            // for walk-transit tours -  if half-tour direction is outbound and stop alternative is in the walk shed, walk and walk-transit should be allowed for ik segments
            //                           if half-tour direction is inbound and stop alternative is in the walk shed, walk and walk-transit should be allowed for kj segments
            //                           if half-tour direction is outbound and stop alternative is in the walk shed, walk and walk-transit should be allowed for kj segments
            //                           if half-tour direction is inbound and stop alternative is in the walk shed, walk and walk-transit should be allowed for ik segments

            // for drive-transit tours - if half-tour direction is outbound and stop alternative is in the drive shed, auto should be allowed for ik segments
            //                           if half-tour direction is inbound and stop alternative is in the drive shed, auto should be allowed for kj segments
            //                           if half-tour direction is outbound and stop alternative is in the walk shed, walk and walk-transit should be allowed for kj segments
            //                           if half-tour direction is inbound and stop alternative is in the walk shed, walk and walk-transit should be allowed for ik segments

            // set values for walk-transit and drive-transit tours according to logic for IK segments
            ...
            // drive transit tours are handled differently than walk transit tours
			...
			ikSegment = logsumHelper.calculateTripMcLogsum( s.getOrig(), altMgra, s.getStopPeriod(), mcModel, mcDmuObject, slcLogger );
            
            
            if ( ikSegment < -900 ) {
                slcLogger.error( "ERROR calculating trip mode choice logsum for " + (s.isInboundStop() ? "inbound" : "outbound") + " stop location choice - ikLogsum = " + ikSegment + "." );
                slcLogger.error( "setting debug to true and recomputing ik segment logsum in order to log utility expression results." );

                if ( s.isInboundStop() )                                
                    slcLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumIBStops=%d, StopOrig=%d, AltStopLoc=%d", s.getTour().getPersonObject().getHouseholdObject().getHhId(), s.getTour().getPersonObject().getPersonNum(), s.getTour().getPersonObject().getPersonType(), s.getTour().getTourPurpose(), s.getTour().getTourModeChoice(), s.getTour().getTourId(), s.getDestPurpose(), "inbound", (s.getStopId()+1), s.getTour().getNumInboundStops()-1, s.getOrig(), altMgra ) );
                else
                    slcLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumOBStops=%d, StopOrig=%d, AltStopLoc=%d", s.getTour().getPersonObject().getHouseholdObject().getHhId(), s.getTour().getPersonObject().getPersonNum(), s.getTour().getPersonObject().getPersonType(), s.getTour().getTourPurpose(), s.getTour().getTourModeChoice(), s.getTour().getTourId(), s.getDestPurpose(), "outbound", (s.getStopId()+1), s.getTour().getNumOutboundStops()-1, s.getOrig(), altMgra ) );
                
                mcDmuObject.getDmuIndexValues().setDebug(true);
                mcDmuObject.getDmuIndexValues().setHHIndex( s.getTour().getPersonObject().getHouseholdObject().getHhId() );
                ikSegment = logsumHelper.calculateTripMcLogsum( s.getOrig(), altMgra, s.getStopPeriod(), mcModel, mcDmuObject, slcLogger );
           ...
           // store the best tap pairs for the segment
            ...
            // if a stop alternative is in the drive shed of a tap, and it's in the inbound direction, the KJ mode HAS TO BE auto.
            //if ( sampleMgraInDriveTransitDriveShed[altMgra] && s.isInboundStop() )
			...
			kjSegment = logsumHelper.calculateTripMcLogsum( altMgra, halfTourFinalDest, s.getStopPeriod(), mcModel, mcDmuObject, slcLogger );

            if ( kjSegment < -900 ) {
                slcLogger.error( "ERROR calculating trip mode choice logsum for " + (s.isInboundStop() ? "inbound" : "outbound") + " stop location choice - kjLogsum = " + kjSegment + "." );
                slcLogger.error( "setting debug to true and recomputing kj segment logsum in order to log utility expression results." );

                if ( s.isInboundStop() )                    
                    slcLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumIBStops=%d, AltStopLoc=%d, HalfTourDest=%d", s.getTour().getPersonObject().getHouseholdObject().getHhId(), s.getTour().getPersonObject().getPersonNum(), s.getTour().getPersonObject().getPersonType(), s.getTour().getTourPurpose(), s.getTour().getTourModeChoice(), s.getTour().getTourId(), s.getDestPurpose(), "inbound", (s.getStopId()+1), s.getTour().getNumInboundStops()-1, altMgra, halfTourFinalDest ) );
                else
                    slcLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumOBStops=%d, AltStopLoc=%d, HalfTourDest=%d", s.getTour().getPersonObject().getHouseholdObject().getHhId(), s.getTour().getPersonObject().getPersonNum(), s.getTour().getPersonObject().getPersonType(), s.getTour().getTourPurpose(), s.getTour().getTourModeChoice(), s.getTour().getTourId(), s.getDestPurpose(), "outbound", (s.getStopId()+1), s.getTour().getNumOutboundStops()-1, altMgra, halfTourFinalDest ) );
    ...
    private void selectSampleOfAlternativesAutoTourNew( Stop s, Tour tour, Person person, Household household, int[] loggingSample ) {
        
        int slcSegment = s.getStopPurposeIndex();
        int slcOrigTaz = mgraManager.getTaz( s.getOrig() );
        int slcDestTaz = -1;
        if ( s.isInboundStop() )
            slcDestTaz = mgraManager.getTaz( tour.getTourOrigMgra() );
        else
            slcDestTaz = mgraManager.getTaz( tour.getTourDestMgra() );
        
        // get sample of locations and correction factors for sample using the alternate method
        dcTwoStageModelObject.chooseSlcSample( slcOrigTaz, slcDestTaz, slcSegment, sampleSize, household.getHhRandom(), household.getDebugChoiceModels() );
    ...
    private void selectSampleOfAlternativesAutoTour( Stop s, Tour tour, Person person, Household household, int[] loggingSample ) {
        ...
        if ( household.getDebugChoiceModels () ) {
            choiceModelDescription = String.format ( "Stop Location SOA Choice Model for: stop purpose=%s, direction=%s, stopId=%d, stopOrig=%d", s.getDestPurpose(), s.isInboundStop() ? "inbound" : "outbound", (s.getStopId()+1), s.getOrig() );
            decisionMakerLabel = String.format ( "HH=%d, persNum=%d, persType=%s, tourId=%d, tourPurpose=%s, tourOrig=%d, tourDest=%d, tourMode=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourId(), tour.getTourPrimaryPurpose(), tour.getTourOrigMgra(), tour.getTourDestMgra(), tour.getTourModeChoice() );
...
//          // set the distance from the stop origin to the final half-tour destination
          stopLocDmuObj.setOrigDestDistance( distanceFromStopOrigToAllMgras[tour.getTourOrigMgra()] );
//
//            // not used in UEC to reference matrices, but may be for debugging using $ORIG and $DEST as an expression
...
//            // not used in UEC to reference matrices, but may be for debugging using $ORIG and $DEST as an expression
        ...
        if( household.getDebugChoiceModels () ){
            ...
            if (loggingSample == null) {
                cm.logUECResultsSpecificAlts( soaLogger, loggingHeader, new int[] { 0, s.getOrig(), tour.getTourOrigMgra(), tour.getTourDestMgra() } );
        ...
        // loop over sampleSize, select alternatives based on probabilitiesList, and count frequency of alternatives chosen.
        ...
        for (int i=0; i < sampleSize; i++) {
            ...
            // write choice model alternative info to log file
            if ( household.getDebugChoiceModels () ) {
                cm.logSelectionInfo ( loggingHeader, String.format("rnCount=%d", rnCount), rn, chosenAlt );
            }
    ...
    private void selectSampleOfAlternativesOther( Stop s, Tour tour, Person person, Household household, int[] loggingSample ) {
        ...
        if ( modelStructure.getTourModeIsWalk(tour.getTourModeChoice()) )
            cm = slcSoaModel[WALK_STOP_LOC_SOA_SHEET_INDEX];
        else if ( modelStructure.getTourModeIsBike(tour.getTourModeChoice()) )
            cm = slcSoaModel[BIKE_STOP_LOC_SOA_SHEET_INDEX];
        else
            cm = slcSoaModel[OTHER_STOP_LOC_SOA_SHEET_INDEX];
...
//            // not used in UEC to reference matrices, but may be for debugging using $ORIG and $DEST as an expression
        ...
        if (cm.getAvailabilityCount() == 0){
            ...
            badSlcLogger.info( "setting stop location sample to be an array with 1 element - just the stop origin mgra." );            
            badSlcLogger.info( "" );            

            household.logHouseholdObject( "Stop Location Choice for trip: HH_" + household.getHhId() + ", Pers_" + tour.getPersonObject().getPersonNum() + ", Tour Purpose_" + tour.getTourPurpose() + ", Tour_" + tour.getTourId() + ", Tour Purpose_" + tour.getTourPurpose() + ", Stop_" + (s.getStopId()+1), badSlcLogger );
            household.logPersonObject( "Stop Location Choice for person " + tour.getPersonObject().getPersonNum(), badSlcLogger, tour.getPersonObject() );
            household.logTourObject("Stop Location Choice for tour " + tour.getTourId(), badSlcLogger, tour.getPersonObject(), tour );
            household.logStopObject("Stop Location Choice for stop " + (s.getStopId()+1), badSlcLogger, s, modelStructure );

            badSlcLogger.info( decisionMakerLabel + " has no available alternatives for " + choiceModelDescription + "." );            
            badSlcLogger.info( "Logging StopLocation SOA Choice utility calculations for: stopOrig=" + s.getOrig() + ", tourOrig=" + tour.getTourOrigMgra() + ", and tourDest=" + tour.getTourDestMgra() + "." );            
            cm.logUECResultsSpecificAlts( badSlcLogger, loggingHeader, new int[] { 0, s.getOrig(), tour.getTourOrigMgra(), tour.getTourDestMgra() } );
            ...
            for (int j=0; j < probabilitiesList.length; j++) {
                ...
                if ( sum != cumProbabilitiesList[j] || sum > 1.0 ) {
                    badSlcLogger.info( "error condition found!  sum=" + sum + ", j=" + j + ", cumProbabilitiesList[j]=" + cumProbabilitiesList[j] );
        ...
        // if the sample was determined for a transit tour, the sample and availability arrays for the full set of SOA alternatives need to be restored.
        if ( modelStructure.getTourModeIsTransit( tour.getTourModeChoice() ) ) {
            for (int i=0; i < soaSample.length; i++) {
    ...
    private void setupLogsumCalculation( Stop s ) {
        ...
        mcDmuObject.setOutboundStops( t.getOutboundStops() == null ? 0 : t.getOutboundStops().length - 1 );
        mcDmuObject.setInboundStops( t.getInboundStops() == null ? 0 : t.getInboundStops().length - 1 );
        
        mcDmuObject.setTripOrigIsTourDest( s.isInboundStop() && s.getStopId() == 0 ? 1 : 0 );
        mcDmuObject.setTripDestIsTourDest( ! s.isInboundStop() && ((s.getStopId() + 1) == (t.getNumOutboundStops() - 1)) ? 1 : 0 );
        ...
        mcDmuObject.setJointTour( t.getTourCategory().equalsIgnoreCase( ModelStructure.JOINT_NON_MANDATORY_CATEGORY ) ? 1 : 0 );
        mcDmuObject.setEscortTour( t.getTourPrimaryPurposeIndex() ==  ModelStructure.ESCORT_PRIMARY_PURPOSE_INDEX ? 1 : 0 );
        ...
        mcDmuObject.setTourModeIsWTran( modelStructure.getTourModeIsWalkLocal(tourMode ) || modelStructure.getTourModeIsWalkPremium(tourMode ) ? 1 : 0 );
            ...
            for (int alt : mgras)
            {
                ...
                for (int[] tapPair : bestTaps ) {
                    ...
                    if ( modelStructure.getTourModeIsWalkTransit(t.getTourModeChoice() ) ) {
                        ...
                        else if ( mgraManager.getTapIsWalkAccessibleFromMgra(alt, tapPair[1]) && mgraManager.getMgrasAreWithinWalkDistance(alt, t.getTourOrigMgra()) ) {
                            accessible = true;
            ...
            for (int alt : mgras)
            {
                ...
                for (int[] tapPair : bestTaps ) {
                    ...
                    if ( modelStructure.getTourModeIsWalkTransit(t.getTourModeChoice() ) ) {
                        ...
                        else if ( mgraManager.getTapIsWalkAccessibleFromMgra(alt, tapPair[1]) && mgraManager.getMgrasAreWithinWalkDistance(alt, t.getTourDestMgra()) ) {
                            accessible = true;
                            sampleMgraInAlightingTapShed[alt] = true;
                        }
                    }
     ...
     * create an array with 1 if the mgra indexed has at least one TAP within walk egress distance of the mgra
     * or zero if no walk TAPS exist for the mgra. 
    private void createWalkTransitAvailableArray() {
        ...
        for (int alt : mgras)
        {
            ...
            // get the TAP set within walk egress distance of the stop location alternative.
            int[] aMgraSet = mgraManager.getMgraWlkTapsDistArray()[alt][0];
    
            // set to 1 if the list of TAPS with walk accessible egress to alt is not empty; 0 otherwise
            if ( aMgraSet != null && aMgraSet.length > 0 )
    ...
    /**
     * Do a monte carlo selection from the array of stored mode choice cumulative probabilities (0 based array).
     * The probabilities were saved at the time the stop location alternative segment mode choice
     * logsums were calculated.  If the stop is not the last stop for the half-tour, the IK segment
     * probabilities are passed in.  If the stop is the last stop, the KJ probabilities are passeed in.
     * 
     * @param household object frim which to get the Random object.
     * @param props is the array of stored mode choice probabilities - 0s based array.
     *  
    ...
    private int selectModeFromProbabilities( Stop s, double[] cumProbs )
    {
        ...
        if ( household.getDebugChoiceModels() || selectedModeAlt < 0 || numAvailAlts >= availAltsToLog ){

            // set the number of available alts to log value to a large number so no more get logged.
            if ( numAvailAlts >= availAltsToLog ) {
                Person person = s.getTour().getPersonObject();
                Tour tour = s.getTour();
                smcLogger.info( "Monte Carlo selection for determining Mode Choice from Probabilities for stop with more than " + availAltsToLog + " mode alts available." );
                smcLogger.info( "HHID=" + household.getHhId() + ", persNum=" + person.getPersonNum() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() + ", tourMode=" + tour.getTourModeChoice() );
                smcLogger.info( "StopID=" + (s.getStopId()+1) + " of " + ( s.isInboundStop() ? tour.getNumInboundStops()-1 : tour.getNumOutboundStops()-1 ) + " stops, inbound=" + s.isInboundStop() + ", stopPurpose=" + s.getDestPurpose() + ", stopDepart=" + s.getStopPeriod() + ", stopOrig=" + s.getOrig() + ", stopDest=" + s.getDest() );
                availAltsToLog = 9999;
            }
            ...
            smcLogger.info("Alternative                      Availability           Utility       Probability           CumProb");
            smcLogger.info("---------------------            ------------       -----------    --------------    --------------");
            ...
            for(int j=0; j < cumProbs.length; j++){
                ...
                smcLogger.info(String.format("%-30s%15s%18s%18.6e%18.6e", altString, "", "", tempProb, cumProbs[j]));
                sumProb += tempProb;
            }

            if ( selectedModeAlt < 0 ) {
                smcLogger.info(" ");
                String altString = String.format( "%-3d %-25s", selectedModeAlt, "no MC alt available" );
                smcLogger.info( String.format("Choice: %s, with rn=%.8f, randomCount=%d", altString, rn, randomCount ) );
                throw new RuntimeException();
            }
            else{
                smcLogger.info(" ");
        ...
        // if this statement is reached, there's a problem with the cumulative probabilities array, so return -1.
        return selectedModeAlt;
    }


    /**
     * This method is taken from the setupStopLocationChoiceAlternativeArrays(), except that the
     * stop location choice dmu attributes are not set and the logsum calculation setup is done
     * only for the selected stop location alternative. 
     * 
     * @param stop object representing the half-tour.
     *  
     * @return the selected mode choice alternative from [1,...,numMcAlts].
     */
    private int getHalfTourModeChoice( Stop s ) {
        ...
        if ( hh.getDebugChoiceModels() ){
            smcLogger.info( "LOGSUM calculation for determining Mode Choice Probabilities for " + (s.isInboundStop() ? "INBOUND" : "OUTBOUND") + " half-tour with no stops." );
            smcLogger.info( "HHID=" + hh.getHhId() + ", persNum=" + s.getTour().getPersonObject().getPersonNum() + ", tourPurpose=" + s.getTour().getTourPrimaryPurpose() + ", tourId=" + s.getTour().getTourId() + ", tourMode=" + s.getTour().getTourModeChoice() );
            smcLogger.info( "StopID=" + (s.getStopId()+1) + ", inbound=" + s.isInboundStop() + ", stopPurpose=" + s.getDestPurpose() + ", stopDepart=" + s.getStopPeriod() + ", stopOrig=" + s.getOrig() + ", stopDest=" + s.getDest() );
        }
        ...
        if ( hh.getDebugChoiceModels() || selectedModeAlt < 0 || mcModel.getAvailabilityCount() >= availAltsToLog ){
            ...
            if ( selectedModeAlt < 0 || mcModel.getAvailabilityCount() >= availAltsToLog ) {
                ...
                if ( mcModel.getAvailabilityCount() >= availAltsToLog ) {
                    availAltsToLog = 9999;
                    smcLogger.info( "Logsum calculation for determining Mode Choice for half-tour more than " + availAltsToLog + " mode alts available." );
                }
                else {
                    smcLogger.info( "Logsum calculation for determining Mode Choice for half-tour with no stops." );
                }
                smcLogger.info( "HHID=" + hh.getHhId() + ", persNum=" + person.getPersonNum() + ", tourPurpose=" + tour.getTourPrimaryPurpose() + ", tourId=" + tour.getTourId() + ", tourMode=" + tour.getTourModeChoice() );
                smcLogger.info( "StopID=" + (s.getStopId()+1) + " of " + ( s.isInboundStop() ? tour.getNumInboundStops()-1 : tour.getNumOutboundStops()-1 ) + " stops, inbound=" + s.isInboundStop() + ", stopPurpose=" + s.getDestPurpose() + ", stopDepart=" + s.getStopPeriod() + ", stopOrig=" + s.getOrig() + ", stopDest=" + s.getDest() );
            }
            ...
            for(int j=0; j < utilities.length; j++){
                ...
                smcLogger.info(String.format("%-30s%15s%18.6e%18.6e%18.6e", altString, availabilities[j+1], utilities[j], probabilities[j], cumProb));
            }
            ...
            if ( selectedModeAlt < 0 ) {
                ...
                smcLogger.info(" ");
                String altString = String.format( "%-3d %-25s", selectedModeAlt, altNames[selectedModeAlt-1] );
                smcLogger.info( String.format("Choice: %s, with rn=%.8f, randomCount=%d", altString, rn, randomCount ) );
            }
            ...
            if ( logsum < -900 || selectedModeAlt < 0 ) {
                if ( logsum < -900 || selectedModeAlt < 0 )
                    smcLogger.error( "ERROR calculating trip mode choice logsum for " + (s.isInboundStop() ? "inbound" : "outbound") + " half-tour with no stops - ikLogsum = " + logsum + "." );
                else    
                    smcLogger.error( "No half-tour mode choice alternatives available " + (s.isInboundStop() ? "inbound" : "outbound") + " half-tour with no stops - ikLogsum = " + logsum + "." );
                smcLogger.error( "setting debug to true and recomputing half-tour logsum in order to log utility expression results." );

                if ( s.isInboundStop() )                    
                    smcLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumIBStops=%d, StopOrig=%d, AltStopLoc=%d", s.getTour().getPersonObject().getHouseholdObject().getHhId(), s.getTour().getPersonObject().getPersonNum(), s.getTour().getPersonObject().getPersonType(), s.getTour().getTourPurpose(), s.getTour().getTourModeChoice(), s.getTour().getTourId(), s.getDestPurpose(), "inbound", (s.getStopId()+1), s.getTour().getNumInboundStops()-1, s.getOrig(), altMgra ) );
                else
                    smcLogger.error( String.format("HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourMode=%d, TourId=%d, StopPurpose=%s, StopDirection=%s, StopId=%d, NumOBStops=%d, StopOrig=%d, AltStopLoc=%d", s.getTour().getPersonObject().getHouseholdObject().getHhId(), s.getTour().getPersonObject().getPersonNum(), s.getTour().getPersonObject().getPersonType(), s.getTour().getTourPurpose(), s.getTour().getTourModeChoice(), s.getTour().getTourId(), s.getDestPurpose(), "outbound", (s.getStopId()+1), s.getTour().getNumOutboundStops()-1, s.getOrig(), altMgra ) );
                
                mcDmuObject.getDmuIndexValues().setDebug(true);
                mcDmuObject.getDmuIndexValues().setHHIndex( s.getTour().getPersonObject().getHouseholdObject().getHhId() );
                logsum = logsumHelper.calculateTripMcLogsum( s.getOrig(), altMgra, s.getStopPeriod(), mcModel, mcDmuObject, smcLogger);
                mcDmuObject.getDmuIndexValues().setDebug(false);
   ...
   private void setOutboundTripDepartTimes ( Stop[] stops ) {
        ...
        for ( int i=0; i < stops.length; i++ ) {
            ...
            if ( tour.getTourArrivePeriod() == tour.getTourDepartPeriod() ) {
                ...
                if ( household.getDebugChoiceModels() ) {
                    ...
                    tripDepartLogger.info( String.format ( "tourDepartPeriod=%d, tourArrivePeriod=%d", tour.getTourDepartPeriod(), tour.getTourArrivePeriod() ) );
                ...
                if ( tripIndex == 1 ) {
                    ...
                    if ( prevTripPeriod == tour.getTourArrivePeriod() ) {
                        ...
                        if ( household.getDebugChoiceModels() ) {
                            tripDepartLogger.info( "Trip Depart Time Model Not Run Since Previous Trip Depart and Tour Arrive Periods are Equal; Stop Depart Period set to Tour Arrive Period = " + tour.getTourArrivePeriod() + " for outbound half-tour." );
                            tripDepartLogger.info( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                            tripDepartLogger.info( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod() ) );
                            tripDepartLogger.info( "" );
                        }
                        stop.setStopPeriod( tour.getTourDepartPeriod() );
                        ...
                        while ( validTripDepartPeriodSet == false ) {
                            ...
                            if ( choice >= prevTripPeriod && choice <= tour.getTourArrivePeriod() ) {
                                ...
                                if ( household.getDebugChoiceModels() ) {
                                    ...
                                    tripDepartLogger.info( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                                    tripDepartLogger.info( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod() ) );
                                    tripDepartLogger.info( "tourPrimaryPurposeIndex=" + tourPrimaryPurposeIndex + ", halfTourDirection=" + halfTourDirection + ", tripIndex=" + tripIndex );
                                    tripDepartLogger.info( "" );
                            ...
                            if ( invalidCount > MAX_INVALID_FIRST_ARRIVAL_COUNT ){
                                ...
                                tripDepartLogger.error( "Outbound trip depart time less than previous trip depart time for " + invalidCount + " times." );
                                tripDepartLogger.error( "Possible infinite loop?" );
                                tripDepartLogger.error( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                                tripDepartLogger.error( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d, last choice=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod(), choice ) );
                                tripDepartLogger.error( "=" + invalidCount + " times." );
                                
                                System.out.println( "Error in Outbound Trip Depart Time Model." );
                                System.out.println( "Outbound trip depart time less than previous trip depart time for " + invalidCount + " times." );
                                System.out.println( "Possible infinite loop?" );
                                System.out.println( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                                System.out.println( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d, last choice=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod(), choice ) );
            ...
            if ( tour.getTourArrivePeriod() == tour.getTourDepartPeriod() ) {
                ...
                if ( tripIndex == 1 ) {
                    ...
                    if ( prevTripPeriod == tour.getTourArrivePeriod() ) {
                        ...
                        if ( household.getDebugChoiceModels() ) {
                            tripDepartLogger.info( "Trip Arrive Time Model Not Run Since Previous Trip Arrive and Tour Arrive Periods are Equal; Stop Arrive Period set to Tour Arrive Period = " + tour.getTourArrivePeriod() + " for intbound half-tour." );
                            tripDepartLogger.info( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                            tripDepartLogger.info( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod() ) );
                            tripDepartLogger.info( "" );
                        }
                        ...
                        while ( validTripArrivePeriodSet == false ) {
                            ...
                            if ( invalidCount > MAX_INVALID_FIRST_ARRIVAL_COUNT ){
                                ...
                                tripDepartLogger.error( "Inbound trip arrive time greater than tour arrive time chosen for " + invalidCount + " times." );
                                tripDepartLogger.error( "Possible infinite loop?" );
                                tripDepartLogger.error( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                                tripDepartLogger.error( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d, last choice=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod(), choice ) );
                                tripDepartLogger.error( "=" + invalidCount + " times." );
                                ...
                                System.out.println( "Inbound trip arrive time greater than tour arrive time chosen for " + invalidCount + " times." );
                                System.out.println( "Possible infinite loop?" );
                                System.out.println( String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourMode=%d, TourCategory=%s, TourPurpose=%s, TourId=%d, StopOrigPurpose=%s, StopDestPurpose=%s, StopId=%d, outboundStopsArray Length=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourModeChoice(), tour.getTourCategory(), tour.getTourPrimaryPurpose(), tour.getTourId(), stop.getOrigPurpose(), stop.getDestPurpose(), (stop.getStopId()+1), stops.length ) );
                                System.out.println( String.format ( "prevTripPeriod=%d, tourDepartPeriod=%d, tourArrivePeriod=%d, last choice=%d", prevTripPeriod, tour.getTourDepartPeriod(), tour.getTourArrivePeriod(), choice ) );
                                System.out.println( "=" + invalidCount + " times." );
    ...
    // this method is called to determine the parking mgra location if the stop location is in parkarea 1 and chosen mode is sov or hov.
    private int selectParkingLocation ( Household household, Tour tour, Stop stop ) {

        Logger modelLogger = parkLocLogger;
        
        // if the trip destination mgra is not in parking area 1, it's not necessary to make a parking location choice
        if ( mgraAltLocationIndex.containsKey( stop.getDest() ) == false || mgraAltParkArea.get( stop.getDest() ) != 1 )
            return -1;
        ...
        if ( tour.getPersonObject().getFreeParkingAvailableResult() == 1 )
            return stop.getDest();
            

        parkingChoiceDmuObj.setDmuIndexValues( household.getHhId(), stop.getOrig(), stop.getDest(), household.getDebugChoiceModels() );
        ...
        if ( stops.length == 1 ) {
            ...
            if ( stopId == stops.length - 1 )
                activityIntervals = tour.getTourArrivePeriod() - stop.getStopPeriod();
            else
                activityIntervals = stops[stopId+1].getStopPeriod() - stop.getStopPeriod();
        }
        ...
        int[] sampleIndices = setupParkLocationChoiceAlternativeArrays( stop.getOrig(), stop.getDest() );               
        
        // if no alternatives in the sample, it's not necessary to make a parking location choice
        ...
        if ( household.getDebugChoiceModels() ) {
            ...
            decisionMakerLabel = String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourPurpose=%s, TourId=%d, StopPurpose=%s, StopId=%d", household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourPurpose(), tour.getTourId(), tour.getTourPurpose(), stop.getStopId() );
            ...
            plcModel.choiceModelUtilityTraceLoggerHeading( choiceModelDescription, decisionMakerLabel );
        
        }

        plcModel.computeUtilities ( parkingChoiceDmuObj, parkingChoiceDmuObj.getDmuIndexValues(), altParkAvail, altParkSample );
        ...
        if ( plcModel.getAvailabilityCount() > 0 ) {
            ...
            // sampleIndices is 1-based, but the values returned are 0-based, parkMgras is 0-based
        ...
        if ( household.getDebugChoiceModels() || chosen < 0 ) {
            ...
            for(int k=1; k <= numAltsInSample; k++){
                ...
                modelLogger.info(String.format("%-35s%18.6e%18.6e%18.6e", altString, utilities[k-1], probabilities[k-1], cumProb));
            }
            ...
            if ( chosen < 0 ) {
                ...
                String altString = String.format( "chosen=%d, chosenIndex=%d, chosenMgra=%d", chosen, chosenIndex, parkMgra );
                modelLogger.info( String.format("Choice: %s, with rn=%.8f, randomCount=%d", altString, rn, randomCount ) );
            }
        ...
        if ( chosen > 0 )
            ...
            logger.error ( String.format( "Exception caught for HHID=%d, personNum=%d, no available parking location alternatives in tourId=%d to choose from in plcModelApplication.", household.getHhId(), person.getPersonNum(), tour.getTourId() ) );
            throw new RuntimeException();
        }
    ...
    // trip destination in parkarea 1 and not a work trip with free onsite parking
    // return false if no parking location alternatives are in walk distance of trip destination; true otherwise.
    private int[] setupParkLocationChoiceAlternativeArrays( int tripOrigMgra, int tripDestMgra ) {
                            
        // get the array of mgras within walking distance of the trip destination
        ...
        if (walkMgras != null)
        {
            ...
            for (int wMgra : walkMgras) {
                // if wMgra is in the set of parkarea==1 MGRAs, add to list of alternatives for this park location choice
                if ( mgraAltLocationIndex.containsKey(wMgra) ) {
                    ...
                    if ( curWalkDist > MgraDataManager.MAX_PARKING_WALK_DISTANCE )
                        continue;
            ...
            if ( altCount > 0 ) {
                ...
                if ( numAltsInSample > maxAltsInSample )
                    maxAltsInSample = numAltsInSample;
            }
