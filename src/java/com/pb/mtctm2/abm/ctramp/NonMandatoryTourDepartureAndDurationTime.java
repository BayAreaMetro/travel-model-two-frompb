...
public class NonMandatoryTourDepartureAndDurationTime implements Serializable {
    ...
    private static final String[] dcModelSheetKeys = {
        IMTOD_UEC_ESCORT_MODEL_TARGET,
        IMTOD_UEC_SHOP_MODEL_TARGET,
        IMTOD_UEC_MAINT_MODEL_TARGET,
        IMTOD_UEC_EAT_MODEL_TARGET,
        IMTOD_UEC_VISIT_MODEL_TARGET,
    ...
    public NonMandatoryTourDepartureAndDurationTime( HashMap<String, String> propertyMap, ModelStructure modelStructure, CtrampDmuFactoryIf dmuFactory, TourModeChoiceModel mcModel ){
    ...
    private void setupTodChoiceModels( HashMap<String, String> propertyMap, CtrampDmuFactoryIf dmuFactory ) {
        ...
        int todModelDataSheet = Util.getIntegerValueFromPropertyMap( propertyMap, IMTOD_UEC_DATA_TARGET );
                    ...
                    try {
                        ...
                        if ( alternativeAvailable < 0 ) {
                            ...
                            if ( runModeChoice ) {
                                ...
                                if ( hh.getDebugChoiceModels() )
                                    hh.logHouseholdObject( "Pre Non-Mandatory Tour Mode Choice Household " + hh.getHhId() + ", Tour " + tourPurpNum + " of " + person.getListOfIndividualNonMandatoryTours().size(), tourMCNonManLogger );
                            ...
                            if ( runModeChoice ) {
                                ...
                                if ( hh.getDebugChoiceModels() )
                                    hh.logHouseholdObject( "Pre Individual Non-Mandatory Tour Mode Choice Household " + hh.getHhId() + ", Tour " + tourPurpNum + " of " + purposeTourLists[tourPurposeIndex].size(), tourMCNonManLogger );
                    ...
                    if ( alternativeAvailable < 0 ) {
                        ...
                        if ( noAltChoice == 1 ) {
                            if ( previouslySelectedDepartPeriod < 0 ) {
                                ...
                                chosenEndPeriod = altEnds[noAltChoice-1];
                                if( hh.getDebugChoiceModels() )
                                    modelLogger.info( "All alternatives already scheduled, and no joint non-mandatory tour scheduled yet, depart AND arrive set to first period=" + chosenStartPeriod + ", " + chosenEndPeriod + "." );
                            }
                            else {
                                chosenStartPeriod = previouslySelectedArrivePeriod;
                                chosenEndPeriod = previouslySelectedArrivePeriod;
                                if( hh.getDebugChoiceModels() )
                                    modelLogger.info( "All alternatives already scheduled, depart AND arrive set to arrive period of most recent scheduled joint non-mandatory tour=" + chosenStartPeriod + ", " + chosenEndPeriod + "." );
                            }
                            noWindowCountFirstTemp++;
                            noAltChoice = departureTimeChoiceAvailability.length-1;
                        }
                            ...
                            if ( previouslySelectedDepartPeriod < 0 ) {
                                ...
                                chosenEndPeriod = altEnds[noAltChoice-1];
                                if( hh.getDebugChoiceModels() )
                                    modelLogger.info( "All alternatives already scheduled, and no joint non-mandatory tour scheduled yet, depart AND arrive set to last period=" + chosenStartPeriod + ", " + chosenEndPeriod + "." );
                            }
                            else {
                                chosenStartPeriod = previouslySelectedArrivePeriod;
                                chosenEndPeriod = previouslySelectedArrivePeriod;
                        ...
                        if ( runModeChoice ) {
                            ...
                            if ( hh.getDebugChoiceModels() )
                                hh.logHouseholdObject( "Pre Joint Non-Mandatory Tour Mode Choice Household " + hh.getHhId() + ", Tour " + tourPurpNum + " of " + purposeTourLists[tourPurposeIndex].size(), tourMCNonManLogger );
    ...
    private void setTourModeChoiceLogsumsForDepartureTimeAndDurationAlternatives( Tour tour, boolean[] altAvailable )
    {
        ...
        String choiceModelDescription = String.format(
                "NonMandatory Tour Mode Choice Logsum calculation for %s Departure Time Choice", tour.getTourPurpose());
        String decisionMakerLabel = String.format( "HH=%d, PersonNum=%d, PersonType=%s, tourId=%d of %d",
                household.getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourId(), person.getListOfWorkTours().size());
...
//         int id = householdDataManager.getArrayIndex( 1033331 );
         ...
         ( propertyMap, modelStructure, ModelStructure.INDIVIDUAL_NON_MANDATORY_CATEGORY, dmuFactory, logsumHelper );
     
         NonMandatoryTourDepartureAndDurationTime testObject = new NonMandatoryTourDepartureAndDurationTime( propertyMap, modelStructure, dmuFactory, inmmcModel );
         ...
         /**
          * used this block of code to test for typos and implemented dmu methiods in the TOD choice UECs
          * 
         String uecFileDirectory = propertyMap.get( CtrampApplication.PROPERTIES_UEC_PATH );
         String todUecFileName = propertyMap.get( IMTOD_UEC_FILE_TARGET );
         todUecFileName = uecFileDirectory + todUecFileName;
         ...
         ModelStructure modelStructure = new SandagModelStructure();
         SandagCtrampDmuFactory dmuFactory = new SandagCtrampDmuFactory(modelStructure);
         TourDepartureTimeAndDurationDMU todDmuObject = dmuFactory.getTourDepartureTimeAndDurationDMU();
         
        int[] indices = { 0, 1, 2, 3, 4, 5 };
         for (int i : indices )
         {
             int uecIndex = i + 4;
             File uecFile = new File(todUecFileName);
             UtilityExpressionCalculator uec = new UtilityExpressionCalculator(uecFile, uecIndex, 0, propertyMap, (VariableTable) todDmuObject);
         }
          */
