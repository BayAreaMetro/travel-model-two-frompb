...
public class CtrampApplication implements Serializable
{
            ...
            CtrampDmuFactoryIf dmuFactory, int globalIterationNumber, float iterationSampleRate)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                serverPort = Integer.parseInt(resourceBundle.getString("RunModel.MatrixServerPort")) + MATRIX_DATA_SERVER_PORT_OFFSET;
        ...
        {
            if (!matrixServerAddress.equalsIgnoreCase("none")) ms.stop32BitMatrixIoServer();
        }
    ...
    private void runModelSequence(int iteration, HouseholdDataManagerIf householdDataManager, CtrampDmuFactoryIf dmuFactory)
    {
        ...
        if (runPreAutoOwnershipChoiceModel)
        {
            ...
            buildNonMandatoryAccessibilities();
                ...
                || runAtWorkSubTourFrequencyModel || runStopFrequencyModel)
        {
            ...
            {
                logJointModelResults(householdDataManager, dmuFactory);
            }
     ...
     *         logsum information
     */
    private void buildNonMandatoryAccessibilities()
    {
    ...
    private MatrixDataServer startMatrixServerProcess(String serverAddress, int serverPort)
    {
        ...
        {
            matrixServer.stop32BitMatrixIoServer();
            logger.error( "RuntimeException caught in ctramp.MatrixDataServer.main() -- exiting.", e);
        }
    ...
    private void logFpResults(HouseholdDataManagerIf householdDataManager)
    {
        ...
        MgraDataManager mgraManager = MgraDataManager.getInstance();
            ...
            for (int i = 0; i < householdArray.length; ++i)
            {
                ...
                for ( int p=1; p < persons.length; p++ ) {
                    ...
                    if ( workLocation > 0 && workLocation != ModelStructure.WORKS_AT_HOME_LOCATION_INDICATOR ) {
                        ...
                        if(area > 0) { //skip unused mgra indexes
