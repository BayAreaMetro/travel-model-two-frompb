...
public class HouseholdChoiceModels
{
    ...
    public HouseholdChoiceModels( int modelIndex, String restartModelString,
            HashMap<String, String> propertyMap, ModelStructure modelStructure, CtrampDmuFactoryIf dmuFactory,
            BuildAccessibilities aggAcc, McLogsumsCalculator logsumHelper, MandatoryAccessibilitiesCalculator mandAcc,
            ...
            double[][][] nonManSoaDistProbs, double[][][] nonManSoaSizeProbs,
            double[][][] subTourSoaDistProbs, double[][][] subTourSoaSizeProbs )
    {
        ...
        try
        {
            useNewSlcSoaMethod = Util.getBooleanValueFromPropertyMap( propertyMap, USE_NEW_SLC_SOA_METHOD_PROPERTY_KEY);
            ...
            if (runAutoOwnershipModel) {
                ...
                if ( measureObjectSizes ) logger.info ( "AO size:         " + ObjectUtil.sizeOf( aoModel ) );
            }

            
            if (runTransponderModel) {                                               
                tcModel = new TransponderChoiceModel(propertyMap, dmuFactory, accTable, pctHighIncome, pctMedHighPlusIncome, pctMultipleAutos, avgtts, transpDist, pctDetour);
                if ( measureObjectSizes ) logger.info ( "TC size:         " + ObjectUtil.sizeOf( tcModel ) );
            }

            
            if ( runParkingProvisionModel ) {
                ppModel = new ParkingProvisionModel( propertyMap, dmuFactory );
                if ( measureObjectSizes ) logger.info ( "PP size:       " + ObjectUtil.sizeOf( ppModel ) );
            }
            ...
            if (runIndividualMandatoryTourFrequencyModel) {
                ...
                if ( measureObjectSizes ) logger.info ( "IMTF size:       " + ObjectUtil.sizeOf( imtfModel ) );
            }

            if ( runMandatoryTourDepartureTimeAndDurationModel || runMandatoryTourModeChoiceModel ) {
                immcModel = new TourModeChoiceModel( propertyMap, modelStructure, TourModeChoiceModel.MANDATORY_MODEL_INDICATOR, dmuFactory, logsumHelper );
                if ( measureObjectSizes ) logger.info ( "IMMC size:       " + ObjectUtil.sizeOf( immcModel ) );
            }
             

            if (runMandatoryTourDepartureTimeAndDurationModel) {
                imtodModel = new HouseholdIndividualMandatoryTourDepartureAndDurationTime(propertyMap, modelStructure, aggAcc.getWorkSegmentNameList(), dmuFactory, immcModel);
                if ( measureObjectSizes ) logger.info ( "IMTOD size:      " + ObjectUtil.sizeOf( imtodModel ) );
            }

            if ( runJointTourFrequencyModel ) {
                jtfModel = new JointTourModels( propertyMap, accTable, modelStructure, dmuFactory );
                if ( measureObjectSizes ) logger.info ( "JTF size:        " + ObjectUtil.sizeOf( jtfModel ) );
            }
             
            if (runIndividualNonMandatoryTourFrequencyModel) {
                inmtfModel = new HouseholdIndividualNonMandatoryTourFrequencyModel(propertyMap, dmuFactory, accTable, mandAcc);
                if ( measureObjectSizes ) logger.info ( "INMTF size:      " + ObjectUtil.sizeOf( inmtfModel ) );
            }

            if (runIndividualNonMandatoryTourLocationChoiceModel || runJointTourLocationChoiceModel
                    || runIndividualNonMandatoryTourDepartureTimeAndDurationModel || runJointTourDepartureTimeAndDurationModel
                    || runIndividualNonMandatoryTourModeChoiceModel|| runJointTourModeChoiceModel) {
                        nmmcModel = new TourModeChoiceModel( propertyMap, modelStructure, TourModeChoiceModel.NON_MANDATORY_MODEL_INDICATOR, dmuFactory, logsumHelper);
                        if ( measureObjectSizes ) logger.info ( "INMMC size:      " + ObjectUtil.sizeOf( nmmcModel ) );
            }
            
            if (runIndividualNonMandatoryTourLocationChoiceModel || runJointTourLocationChoiceModel) {
                nmlcModel = new NonMandatoryDestChoiceModel( propertyMap, modelStructure, aggAcc, dmuFactory, nmmcModel );
                nmlcModel.setNonMandatorySoaProbs( nonManSoaDistProbs, nonManSoaSizeProbs );
                if ( measureObjectSizes ) logger.info ( "INMLC size:      " + ObjectUtil.sizeOf( nmlcModel ) );
            }

            if (runIndividualNonMandatoryTourDepartureTimeAndDurationModel || runJointTourDepartureTimeAndDurationModel) {
                nmtodModel = new NonMandatoryTourDepartureAndDurationTime( propertyMap, modelStructure, dmuFactory, nmmcModel );
                if ( measureObjectSizes ) logger.info ( "INMTOD size:     " + ObjectUtil.sizeOf( nmtodModel ) );
            }
            
            if (runAtWorkSubTourFrequencyModel) {
                awfModel = new HouseholdAtWorkSubtourFrequencyModel(propertyMap, modelStructure, dmuFactory);
                if ( measureObjectSizes ) logger.info ( "AWTF size:       " + ObjectUtil.sizeOf( awfModel ) );
            }

            if ( runAtWorkSubtourLocationChoiceModel
                    || runAtWorkSubtourDepartureTimeAndDurationModel
                    || runAtWorkSubtourModeChoiceModel ) {
                         awmcModel = new TourModeChoiceModel( propertyMap, modelStructure, TourModeChoiceModel.AT_WORK_SUBTOUR_MODEL_INDICATOR, dmuFactory, logsumHelper );
                         if ( measureObjectSizes ) logger.info ( "AWMC size:       " + ObjectUtil.sizeOf( awmcModel ) );
            }

            if ( runAtWorkSubtourLocationChoiceModel ) {
                awlcModel = new SubtourDestChoiceModel( propertyMap, modelStructure, aggAcc, dmuFactory, awmcModel );
                awlcModel.setNonMandatorySoaProbs( subTourSoaDistProbs, subTourSoaSizeProbs );
                if ( measureObjectSizes ) logger.info ( "AWLC size:       " + ObjectUtil.sizeOf( awlcModel ) );
            }

            if ( runAtWorkSubtourDepartureTimeAndDurationModel ) {
                awtodModel = new SubtourDepartureAndDurationTime( propertyMap, modelStructure, dmuFactory, awmcModel );
                if ( measureObjectSizes ) logger.info ( "AWTOD size:      " + ObjectUtil.sizeOf( awtodModel ) );
            }

            if (runStopFrequencyModel) {
                stfModel = new StopFrequencyModel(propertyMap, dmuFactory, modelStructure, accTable);
                if ( measureObjectSizes ) logger.info ( "STF size:        " + ObjectUtil.sizeOf( stfModel ) );
            }

            if ( runStopLocationModel ) {
                stlmcModel = new IntermediateStopChoiceModels( propertyMap, modelStructure, dmuFactory, logsumHelper );

                
                // if the slcTazDistProbs are not null, they have been already computed, and it is
                // not necessary for the thread creating this HouseholdChoiceModels object to 
                ...
                if ( useNewSlcSoaMethod && slcSizeProbs == null ) {
                    ...
                    DestChoiceTwoStageSoaTazDistanceUtilityDMU locChoiceDistSoaDmu = dmuFactory.getDestChoiceSoaTwoStageTazDistUtilityDMU();        
                    
                    DestChoiceTwoStageSoaProbabilitiesCalculator slcSoaDistProbsObject =  new DestChoiceTwoStageSoaProbabilitiesCalculator(
                            propertyMap, dmuFactory,
                    ...
                    stlmcModel.setupSlcDistanceBaseSoaModel( propertyMap, slcTazDistExpUtils, slcSizeProbs, slcTazSize );                    
                }

                if ( measureObjectSizes ) logger.info ( "SLMT size:       " + ObjectUtil.sizeOf( stlmcModel ) );
            }
        ...
        } catch (RuntimeException e)
        {
            ...
            if ( runParkingProvisionModel && ppModel != null )
                lastModel += " fp";
            ...
            if ( runJointTourModeChoiceModel && nmmcModel != null )
                lastModel += " jmc";

            if ( runJointTourLocationChoiceModel && nmlcModel != null )
                lastModel += " jlc";
    ...
    public void runModelsWithTiming(Household hhObject)
    {
        ...
        if ( runMandatoryTourDepartureTimeAndDurationModel ) {
            long check = System.nanoTime();
            //long hhSeed = globalSeed + hhObject.getHhId() + IMTOD_SEED_OFFSET; 
            //hhObject.getHhRandom().setSeed( hhSeed );
            imtodModel.applyModel( hhObject, runMandatoryTourModeChoiceModel );
        ...
        if ( runIndividualNonMandatoryTourFrequencyModel ) {
            long check = System.nanoTime();
            //long hhSeed = globalSeed + hhObject.getHhId() + INMTF_SEED_OFFSET; 
            //hhObject.getHhRandom().setSeed( hhSeed );
            inmtfModel.applyModel( hhObject );
        ...
        if ( runIndividualNonMandatoryTourLocationChoiceModel ) {
            long check = System.nanoTime();
            //long hhSeed = globalSeed + hhObject.getHhId() + INMDC_SEED_OFFSET; 
            //hhObject.getHhRandom().setSeed( hhSeed );
        ...
        if ( runIndividualNonMandatoryTourDepartureTimeAndDurationModel ) {
            long check = System.nanoTime();
            //long hhSeed = globalSeed + hhObject.getHhId() + INMTOD_SEED_OFFSET; 
            //hhObject.getHhRandom().setSeed( hhSeed );
            nmtodModel.applyIndivModel( hhObject, runIndividualNonMandatoryTourModeChoiceModel );
        ...
        if ( runAtWorkSubtourDepartureTimeAndDurationModel ) {
            long check = System.nanoTime();
            //long hhSeed = globalSeed + hhObject.getHhId() + AWTOD_SEED_OFFSET; 
            //hhObject.getHhRandom().setSeed( hhSeed );
            awtodModel.applyModel( hhObject, runAtWorkSubtourModeChoiceModel );
    ...
    private void checkRestartModel(Household hhObject)
    {
        ...
        // version 1.0.8.22 - changed model restart options - possible values for
        // restart are now: none, uwsl, ao, imtf, jtf, inmtf, stf

        // if restartModel was specified, reset the random number sequence
        // based on the cumulative count of random numbers drawn by the component
    ...
    private void computeSlcSoaProbabilities( DestChoiceTwoStageSoaProbabilitiesCalculator locChoiceSoaDistProbsObject,
        ...
        slcTazDistExpUtils = computeTazDistanceExponentiatedUtilities( locChoiceSoaDistProbsObject, locChoiceDistSoaDmu );
        ...
        for ( String segmentName : segmentNameIndexMap.keySet() ) {
            ...
            slcSizeProbs[segmentIndex] = computeSizeSegmentProbabilities( dcSizeArray[segmentIndex], slcTazSize[segmentIndex] );  
    ...
    private double[][] computeTazDistanceExponentiatedUtilities( DestChoiceTwoStageSoaProbabilitiesCalculator locChoiceSoaDistProbsObject,
        ...
        // compute the TAZ x TAZ exponentiated utilities array for sample selection utilities.
