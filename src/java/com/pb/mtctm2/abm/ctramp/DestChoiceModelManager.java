        ...
        implements Serializable
{
    ...
    private transient Logger                     logger              = Logger.getLogger(DestChoiceModelManager.class);
    ...
    // the first dimension on these arrays is work location segments (worker occupations)
    private double[][][]                         workSizeProbs;
    private double[][][]                         workTazDistProbs;        

    // the first dimension on these arrays is school location segment type (ps, gs, hs, univTypical, univNonTypical)
    private double[][][]                         schoolSizeProbs;
    private double[][][]                         schoolTazDistProbs;        

    private AutoTazSkimsCalculator               tazDistanceCalculator;
    
    private boolean                              managerIsSetup;
    ...
    public synchronized void managerSetup(HashMap<String, String> propertyMap, ModelStructure modelStructure, MatrixDataServerIf ms,
            String dcUecFileName, String soaUecFileName, int soaSampleSize, CtrampDmuFactoryIf dmuFactory, String restartModelString)
    {
        ...
        if (ms == null)
        {
            if (mdm != null)
            {
                logger.info(Thread.currentThread().getName()
                      + ": No remote MatrixServer being used, but MatrixDataManager already exists - resetting for DestChoiceModelManager.");
                mdm.clearData();
            } else
            {
                logger.info(Thread.currentThread().getName()
        ...
        } else if (mdm == null)
        {
            ...
            logger.info(String.format(Thread.currentThread().getName() + ": DestChoiceModelManager needs MatrixDataManager."));
    ...
    public synchronized void returnWorkLocModelObject(WorkLocationChoiceModel dcModel, int taskIndex, int startIndex, int endIndex) {        
        modelQueueWorkLoc.add(dcModel);
        completedHouseholdsWork += (endIndex - startIndex + 1);
        logger.info(String.format(
            "returned workLocationChoice[%d,%d] to workQueueLoc, task=%d, thread=%s, completedHouseholds=%d.", currentIteration, dcModel.getModelIndex(),
            taskIndex, Thread.currentThread().getName(), completedHouseholdsWork));
    }

    public synchronized void returnDcWorkModelObject(MandatoryDestChoiceModel dcModel, int taskIndex, int startIndex, int endIndex) {        
        modelQueueWork.add(dcModel);
        completedHouseholdsWork += (endIndex - startIndex + 1);
        logger.info(String.format(
            "returned dcModelWork[%d,%d] to workQueue, task=%d, thread=%s, completedHouseholds=%d.", currentIteration, dcModel.getModelIndex(),
            taskIndex, Thread.currentThread().getName(), completedHouseholdsWork));
    }

    public synchronized void returnSchoolLocModelObject(SchoolLocationChoiceModel dcModel, int taskIndex, int startIndex, int endIndex) {        
        modelQueueSchoolLoc.add(dcModel);
        completedHouseholdsSchool += (endIndex - startIndex + 1);
        logger.info(String.format(
            "returned schoolLocationChoice[%d,%d] to schoolQueueLoc, task=%d, thread=%s, completedHouseholds=%d.", currentIteration, dcModel.getModelIndex(),
            taskIndex, Thread.currentThread().getName(), completedHouseholdsSchool));
    }
    
    public synchronized void returnDcSchoolModelObject(MandatoryDestChoiceModel dcModel, int taskIndex, int startIndex, int endIndex) {        
        modelQueueSchool.add(dcModel);
        completedHouseholdsSchool += (endIndex - startIndex + 1);
        logger.info(String.format(
            "returned dcModelSchool[%d,%d] to schoolQueue, task=%d, thread=%s, completedHouseholds=%d.", currentIteration, dcModel.getModelIndex(),
            taskIndex, Thread.currentThread().getName(), completedHouseholdsSchool));
    }
    
    public synchronized WorkLocationChoiceModel getWorkLocModelObject( int taskIndex, int iteration, DestChoiceSize dcSizeObj, int[] uecIndices, int[] soaUecIndices )
    {
        
        // can release memory for the school location choice probabilities before running school location choice
        ...
        if ( ! modelQueueWorkLoc.isEmpty() ) {
            ...
            // the first task processed with an iteration parameter greater than the manager's
            // current iteration updates the manager's SOA size and dist probabilities arrays and
            // updates the iteration count.
            if ( iteration > currentIteration ) {
                
                // update the arrays of cumulative probabilities based on mgra size for mgras within each origin taz.        
            ...
            logger.info( String.format("removed workLocationChoice[%d,%d] from workQueueLoc, task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
            ...
            if ( modelIndexWork == 0 && iteration == 0 ) {                
                ...
                workLocSoaDistProbsObject =  new DestChoiceTwoStageSoaProbabilitiesCalculator( propertyMap, dmuFactory,
                        PROPERTIES_WORK_DC_SOA_UEC_FILE, PROPERTIES_WORK_DC_SOA_UEC_MODEL_PAGE, PROPERTIES_WORK_DC_SOA_UEC_DATA_PAGE );                      
                double[][] dcSizeArray = dcSizeObj.getDcSizeArray();
            ...
            TourModeChoiceModel immcModel = new TourModeChoiceModel( propertyMap, null, TourModeChoiceModel.MANDATORY_MODEL_INDICATOR, dmuFactory, logsumHelper );

            

            dcModel = new WorkLocationChoiceModel ( modelIndexWork, propertyMap, dcSizeObj, aggAcc,
                    dcUecFileName, soaUecFileName, soaSampleSize, modeChoiceUecFileName, dmuFactory, immcModel,
                    workSizeProbs, workTazDistProbs );

            dcModel.setupWorkSegments(uecIndices, soaUecIndices);
            dcModel.setupDestChoiceModelArrays( propertyMap, dcUecFileName, soaUecFileName, soaSampleSize );

            logger.info( String.format("created workLocationChoice[%d,%d], task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
    ...
    public synchronized MandatoryDestChoiceModel getDcWorkModelObject( int taskIndex, int iteration, DestChoiceSize dcSizeObj, int[] uecIndices, int[] soaUecIndices )
    {
        ...
        if ( ! modelQueueWork.isEmpty() ) {
            ...
            logger.info( String.format("removed dcModelWork[%d,%d] from workQueue, task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
            ...
            dcModel = new MandatoryDestChoiceModel(modelIndexWork, propertyMap, dcSizeObj,
                    aggAcc, mgraManager, dcUecFileName, soaUecFileName, soaSampleSize, modeChoiceUecFileName, dmuFactory, immcModel );

            dcModel.setupWorkSegments(uecIndices, soaUecIndices);
            dcModel.setupDestChoiceModelArrays( propertyMap, dcUecFileName, soaUecFileName, soaSampleSize );

            logger.info( String.format("created dcModelWork[%d,%d], task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
    ...
    public synchronized SchoolLocationChoiceModel getSchoolLocModelObject( int taskIndex, int iteration, DestChoiceSize dcSizeObj )
    {
        // can release memory for the work location choice probabilities before running school location choice
        ...
        if ( ! modelQueueSchoolLoc.isEmpty() ) {
            ...
            // the first task processed with an iteration parameter greater than the
            // manager's current iteration count clears the dcModel cache and
            // updates the iteration count.
            if ( iteration > currentIteration ) {

                // compute the exponentiated distance utilities that all segments of this tour purpose will share
                ...
                updateSchoolSoaProbabilities( aggAcc.getPsSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                        schoolSizeProbs[PRESCHOOL_ALT_INDEX], schoolTazDistProbs[PRESCHOOL_ALT_INDEX] );
                ...
                updateSchoolSoaProbabilities( aggAcc.getGsSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                        schoolSizeProbs[GRADE_SCHOOL_ALT_INDEX], schoolTazDistProbs[GRADE_SCHOOL_ALT_INDEX] );
                ...
                updateSchoolSoaProbabilities( aggAcc.getHsSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                        schoolSizeProbs[HIGH_SCHOOL_ALT_INDEX], schoolTazDistProbs[HIGH_SCHOOL_ALT_INDEX] );
                ...
                updateSchoolSoaProbabilities( aggAcc.getUnivTypicalSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                        schoolSizeProbs[UNIV_TYPICAL_ALT_INDEX], schoolTazDistProbs[UNIV_TYPICAL_ALT_INDEX] );
                
                logger.info( "updating university-non-typical school SOA Distance and Size probabilities." );
                updateSchoolSoaProbabilities( aggAcc.getUnivNonTypicalSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                        schoolSizeProbs[UNIV_NONTYPICAL_ALT_INDEX], schoolTazDistProbs[UNIV_NONTYPICAL_ALT_INDEX] );
            ...
            logger.info( String.format("removed schoolLocationChoice[%d,%d] from schoolQueueLoc, task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
            ...
            if ( modelIndexSchool == 0 && iteration == 0 ) {                
                ...
                // if the schoolSizeProbs array is null, no task has yet initialized the probabilities arrays, so enter the block.
                // if not null, the arrays have been computed, so it's ok to skip.
                if ( schoolSizeProbs == null ) {
                    ...
                    try {
                        ...
                        psLocSoaDistProbsObject =  new DestChoiceTwoStageSoaProbabilitiesCalculator( propertyMap, dmuFactory,
                                PROPERTIES_PS_DC_SOA_UEC_FILE, PROPERTIES_PS_DC_SOA_UEC_MODEL_PAGE, PROPERTIES_PS_DC_SOA_UEC_DATA_PAGE );                      
                        tazDistExpUtils = computeTazDistanceExponentiatedUtilities( psLocSoaDistProbsObject );
                        updateSchoolSoaProbabilities( aggAcc.getPsSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils, 
                                schoolSizeProbs[PRESCHOOL_ALT_INDEX], schoolTazDistProbs[PRESCHOOL_ALT_INDEX] );
                    }
                    ...
                    try {
                        ...
                        gsLocSoaDistProbsObject =  new DestChoiceTwoStageSoaProbabilitiesCalculator( propertyMap, dmuFactory,
                                PROPERTIES_GS_DC_SOA_UEC_FILE, PROPERTIES_GS_DC_SOA_UEC_MODEL_PAGE, PROPERTIES_GS_DC_SOA_UEC_DATA_PAGE );                          
                        tazDistExpUtils = computeTazDistanceExponentiatedUtilities( gsLocSoaDistProbsObject );
                        updateSchoolSoaProbabilities( aggAcc.getGsSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                                schoolSizeProbs[GRADE_SCHOOL_ALT_INDEX], schoolTazDistProbs[GRADE_SCHOOL_ALT_INDEX] );
                    }
                    ...
                    try {
                        ...
                        hsLocSoaDistProbsObject =  new DestChoiceTwoStageSoaProbabilitiesCalculator( propertyMap, dmuFactory,
                                PROPERTIES_HS_DC_SOA_UEC_FILE, PROPERTIES_HS_DC_SOA_UEC_MODEL_PAGE, PROPERTIES_HS_DC_SOA_UEC_DATA_PAGE );                      
                        tazDistExpUtils = computeTazDistanceExponentiatedUtilities( hsLocSoaDistProbsObject );
                        updateSchoolSoaProbabilities( aggAcc.getHsSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                                schoolSizeProbs[HIGH_SCHOOL_ALT_INDEX], schoolTazDistProbs[HIGH_SCHOOL_ALT_INDEX] );
                    }
                    ...
                    try {
                        ...
                        univLocSoaDistProbsObject =  new DestChoiceTwoStageSoaProbabilitiesCalculator( propertyMap, dmuFactory,
                                PROPERTIES_UNIV_DC_SOA_UEC_FILE, PROPERTIES_UNIV_DC_SOA_UEC_MODEL_PAGE, PROPERTIES_UNIV_DC_SOA_UEC_DATA_PAGE );                          
                        tazDistExpUtils = computeTazDistanceExponentiatedUtilities( univLocSoaDistProbsObject );
                        updateSchoolSoaProbabilities( aggAcc.getUnivTypicalSegmentNameIndexMap(), dcSizeObj, tazDistExpUtils,
                                schoolSizeProbs[UNIV_TYPICAL_ALT_INDEX], schoolTazDistProbs[UNIV_TYPICAL_ALT_INDEX] );
                    }
            ...
            dcModel = new SchoolLocationChoiceModel( modelIndexSchool, propertyMap, dcSizeObj, aggAcc,
                    dcUecFileName, soaUecFileName, soaSampleSize, modeChoiceUecFileName, dmuFactory, immcModel,
                    schoolSizeProbs, schoolTazDistProbs );

            dcModel.setupSchoolSegments();
            dcModel.setupDestChoiceModelArrays( propertyMap, dcUecFileName, soaUecFileName, soaSampleSize );

            logger.info( String.format("created schoolLocationChoice[%d,%d], task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
    ...
    public synchronized MandatoryDestChoiceModel getDcSchoolModelObject( int taskIndex, int iteration, DestChoiceSize dcSizeObj )
    {
        ...
        if ( ! modelQueueSchool.isEmpty() ) {
            ...
            logger.info( String.format("removed dcModelSchool[%d,%d] from schoolQueue, task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
            ...
            logger.info( String.format("created dcModelSchool[%d,%d], task=%d, thread=%s.", currentIteration, dcModel.getModelIndex(), taskIndex, Thread.currentThread().getName()) );
    ...
    private void clearWorkLocModels()
    {
        ...
        if ( modelQueueWorkLoc != null ) {
            
            System.out.println(String.format( "%s:  clearing dc choice models modelQueueWorkLoc, thread=%s.", new Date(), Thread.currentThread().getName()));
            ...
            modelQueueWorkLoc = null;

        }

        if ( modelQueueWork != null ) {
            
            System.out.println(String.format( "%s:  clearing dc choice models modelQueueWork, thread=%s.", new Date(), Thread.currentThread().getName()));
    ...
    private void clearSchoolLocModels()
    {
        ...
        if ( modelQueueSchoolLoc != null ) {
            
            System.out.println(String.format( "%s:  clearing dc choice models modelQueueSchoolLoc, thread=%s.", new Date(), Thread.currentThread().getName()));
            ...
            modelQueueSchoolLoc = null;

        }

        if ( modelQueueSchool != null ) {
            
            System.out.println(String.format( "%s:  clearing dc choice models modelQueueSchool, thread=%s.", new Date(), Thread.currentThread().getName()));
    ...
    private void updateWorkSoaProbabilities( DestChoiceTwoStageSoaProbabilitiesCalculator  locChoiceSoaDistProbsObject,
            DestChoiceSize dcSizeObj, double[][][] sizeProbs,
            double[][][] tazDistProbs, double[][] dcSizeArray ) {
        ...
        for ( String segmentName : segmentNameIndexMap.keySet() ) {
            ...
            //tazDistProbs[segmentIndex] = locChoiceSoaDistProbsObject.computeDistanceProbabilities( 3737, locChoiceDistSoaDmu );
            tazDistProbs[segmentIndex] = locChoiceSoaDistProbsObject.computeDistanceProbabilities( locChoiceDistSoaDmu );
    ...
    private void updateSchoolSoaProbabilities( HashMap<String, Integer> segmentNameIndexMap, DestChoiceSize dcSizeObj,
            double[][] tazDistExpUtils, double[][] sizeProbs, double[][] tazDistProbs ) {
        ...
        for ( String segmentName : segmentNameIndexMap.keySet() ) {
            ...
            // compute the taz dist probabilities from the exponentiated utilities for this segmnet and the taz size terms
            for ( int i=0; i < tazDistExpUtils.length; i++ ) {
                
                // compute the final exponentiated utilities by multiplying with taz size, and accumulate total exponentiated utility.
    ...
    private double[][] computeTazDistanceExponentiatedUtilities( DestChoiceTwoStageSoaProbabilitiesCalculator locChoiceSoaDistProbsObject ) {

        double[][] tazDistExpUtils = locChoiceSoaDistProbsObject.computeDistanceUtilities( locChoiceDistSoaDmu );
        for ( int i=0; i < tazDistExpUtils.length; i++ )
            for ( int j=0; j < tazDistExpUtils[i].length; j++ ) {
                if ( tazDistExpUtils[i][j] < -500 )
                    tazDistExpUtils[i][j] = 0;
                else
                    tazDistExpUtils[i][j] = Math.exp( tazDistExpUtils[i][j] );
            }
