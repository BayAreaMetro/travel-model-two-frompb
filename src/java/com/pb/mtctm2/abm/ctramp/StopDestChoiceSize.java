 ...
 */
public class StopDestChoiceSize
        implements Serializable
{
    ...
    private final Map<String, Map<String, Map<Integer, Double>>> sizeMap;                                                                           // map
    // of
    // purpose,purpose
    // segment,
    // and
    ...
    public StopDestChoiceSize(HashMap<String, String> propertyMap, TazDataIf tazDataManager,
    ...
    private TableDataSet readSizeCoefficientTable(String coeffsFileName)
    {
        try
        {
            ...
            return reader.readFile(new File(coeffsFileName));
        } catch (Exception e)
        {
            logger.fatal(String.format(
                    "Exception occurred reading DC Stop Size coefficients data file = %s.",
                    coeffsFileName), e);
    ...
    private void loadSizeCoefficientTableInformation(TableDataSet coefficients)
    {
        ...
        {
            logger.fatal("Purpose column (" + purposeColumn
                    + ") not found in stop size coefficient table");
            errors = true;
        }
        if (!foundSegmentColumn)
        {
            logger.fatal("Purpose segment column (" + segmentColumn
                    + ") not found in stop size coefficient table");
            errors = true;
        }
        ...
        if (!errors)
        {
            ...
            for (int i = 1; i <= coefficients.getRowCount(); i++)
            {
                ...
                if (validPurposes.contains(purpose))
                {
                    ...
                    {
                        logger.fatal("Invalid segment for purpose " + purpose
                                + " found in stop destination choice size coefficient table: "
                                + segment);
                        errors = true;
                    }
            ...
            for (String segment : sizeCoefficients.get(purpose).keySet())
            {
                ...
                for (int i = 1; i <= tazDataManager.getNumberOfZones(); i++)
                {
                    ...
                    for (String column : sizeCoefficients.get(purpose).get(segment).keySet())
                        size += tazDataManager.getZoneTableValue(i, column)
