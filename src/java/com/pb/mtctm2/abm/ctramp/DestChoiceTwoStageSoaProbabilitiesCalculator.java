...
public class DestChoiceTwoStageSoaProbabilitiesCalculator
{
    ...
    public DestChoiceTwoStageSoaProbabilitiesCalculator( HashMap<String,String> propertyMap, CtrampDmuFactoryIf dmuFactory,
            String soaDistUECPropertyKey, String soaDistUECModelSheetPropertyKey, String soaDistUECDataSheetPropertyKey ) {
        ...
        DestChoiceTwoStageSoaTazDistanceUtilityDMU tazDistUtilityDmu = dmuFactory.getDestChoiceSoaTwoStageTazDistUtilityDMU();
        
        // create a ChoiceModelApplication object for the filename, model page and data page.
        cm = new ChoiceModelApplication( soaDistUecFileName, soaModelPage, soaDataPage, propertyMap, (VariableTable)tazDistUtilityDmu );
    ...
    /**
     * @param dmuObject is the distance utility DMU object
     */
    ...
    /**
     * @param dmuObject is the distance utility DMU object
     * @param distUtilityIndex is the distance utility segment index
     * This method signature is the default, assuming that no distance probabilities logging is required
     */
    public double[][] computeDistanceProbabilities( DestChoiceTwoStageSoaTazDistanceUtilityDMU dmuObject ) {
        ...
        for ( int i=0; i < tazDistProbs.length; i++ ) {
            ...
            iv.setZoneIndex( i+1 ); 
            cm.computeUtilities( dmuObject, iv );
            double[] tempArray = Arrays.copyOf( cm.getCumulativeProbabilities(), tazDistProbs.length );
     ...
     * @param dmuObject is the distance utility DMU object
     * @param distUtilityIndex is the distance utility segment index
     * This alternative method signature allows distance probabilities logging to be written
     */
    public double[][] computeDistanceProbabilities( int traceOrig, DestChoiceTwoStageSoaTazDistanceUtilityDMU dmuObject ) {
        ...
        for ( int i=0; i < tazDistProbs.length; i++ ) {
            ...
            if ( i == traceOrig-1 ) {
                ...
                cm.logUECResultsSpecificAlts( soaTwoStageProbsLogger, "Two stage SOA Dist Utilities from TAZ = " + (i+1), altsToLog );
                ...
                for ( int k=1; k < altsToLog.length; k++ )
                    soaTwoStageProbsLogger.info( "alt=" + (altsToLog[k]-1) + ", util=" + utils[altsToLog[k]-1] + ", prob=" + probs[altsToLog[k]-1] );
        ...
        for ( int i=0; i < tazDistProbs.length; i++ ) {
            soaTwoStageProbsLogger.info( "orig=" + (i+1) + ", dest=3738, cumProb[3737]=" + tazDistProbs[i][3736] + ", cumProb[3738]=" + tazDistProbs[i][3737] + ", prob[3738]=" + (tazDistProbs[i][3737] - tazDistProbs[i][3736]) );
        }
