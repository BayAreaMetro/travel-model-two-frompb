...
public class ControlFileReader implements Serializable {
        ...
        while (true) {
            ...
            for (int col=0; col < header.numberOfAlts; col++) {
                ...
                if ( (str != null) && (str.length() > 0) )
                    coeff[col] = Float.parseFloat( str.trim() );
                else
                    coeff[col] = 0;
    ...
    protected ArrayList readTableDataEntries() {
        ...
        int row = findEntry(sheet, "Table Data", 0, 0, false);
        int mtxRow = findEntry(sheet, "Matrix Data", 0, 0, false);
    ...
    protected ArrayList readMatrixDataEntries() {
		...
		int row = findEntry(sheet, "1", mtxRow, 0, false);
		if (row == -1) {
