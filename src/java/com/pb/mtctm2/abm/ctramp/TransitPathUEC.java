...
import com.pb.mtctm2.abm.ctramp.Modes.TransitMode;
        ...
        implements Serializable
{
    ...
    {
        uec = new UtilityExpressionCalculator(uecSpreadsheet, modelSheet, dataSheet, rbMap, dmu);
    }
    ...
    public boolean comparePaths(double[] calculatedUtilities, int pTap, int aTap, boolean trace)
    {
        ...
        for (int i = 0; i < bestUtilities.length; i++)
        {
            ...
            if (trace)
            {
                logger.info("Mode " + i + " calculatedUtility " + calculatedUtilities[i]
            ...
            if (calculatedUtilities[i] > bestUtilities[i])
            {
                ...
                if (trace)
                {
                    ...
                    logger.info("Best utility so far for mode " + tm[i] + " pTap " + pTap
    ...
    public void createDriveAccessUEC(File uecSpreadsheet, int modelSheet, int dataSheet,
            HashMap<String, String> rbMap, VariableTable dmu)
    {
        driveAccessUEC = new UtilityExpressionCalculator(uecSpreadsheet, modelSheet, dataSheet,
                rbMap, dmu);
     ...
     */
    public double[] calculateDriveUtilityForTransit(TazDataManager tazManager, int itdz, int pos,
