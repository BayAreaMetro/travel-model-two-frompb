        ...
        implements Serializable
{
            ...
            String myTourCategory, CtrampDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper)
    {
        ...
        //logsumHelper passed in, but if it were instantiated here, it woul be as follows
    ...
    private void setupModeChoiceModelApplicationArray(HashMap<String, String> propertyMap, String tourCategory)
    {
        ...
        else if (tourCategory.equalsIgnoreCase(NON_MANDATORY_MODEL_INDICATOR))
        {
            ...
            int maintSheet = Integer.parseInt(propertyMap.get(PROPERTIES_UEC_MAINT_TOUR_MODE_SHEET));
            int discrSheet = Integer.parseInt(propertyMap.get(PROPERTIES_UEC_DISCR_TOUR_MODE_SHEET));
            ...
            for (String purposeName : tourPurposeList)
            {
                ...
                if ( purposeName.equalsIgnoreCase( ModelStructure.ESCORT_PRIMARY_PURPOSE_NAME ) ||
                        purposeName.equalsIgnoreCase( ModelStructure.SHOPPING_PRIMARY_PURPOSE_NAME ) ||
                        purposeName.equalsIgnoreCase( ModelStructure.OTH_MAINT_PRIMARY_PURPOSE_NAME )  )
                    uecIndex = maintSheet;
                else if ( purposeName.equalsIgnoreCase( ModelStructure.EAT_OUT_PRIMARY_PURPOSE_NAME ) ||
                        purposeName.equalsIgnoreCase( ModelStructure.VISITING_PRIMARY_PURPOSE_NAME ) ||
        ...
        else if (tourCategory.equalsIgnoreCase(AT_WORK_SUBTOUR_MODEL_INDICATOR))
        {
            ...
            uecSheets[0] = Integer.parseInt(propertyMap.get(PROPERTIES_UEC_AT_WORK_TOUR_MODE_SHEET));
        ...
        // declare dimensions for the array of choice alternative availability by
        // purpose
        modeAltNames = new String[purposeModelIndexMap.size()][];

        // for each unique model index, create the ChoiceModelApplication object and
        // the availabilty array
        int i = 0;
        for (int m : modelIndexMap.keySet())
        {
            mcModel[i] = new ChoiceModelApplication(mcUecFile, m, MC_DATA_SHEET, propertyMap, (VariableTable) mcDmuObject);
            ...
            Logger modelLogger, String choiceModelDescription, String decisionMakerLabel)
    {
        ...
        double logsum = logsumHelper.calculateTourMcLogsum( tour.getTourOrigMgra(), tour.getTourDestMgra(), tour.getTourDepartPeriod(), tour.getTourArrivePeriod(), mcModel[modelIndex], mcDmuObject );
    ...
    public int getModeChoice(TourModeChoiceDMU mcDmuObject, String purposeName)
    {
            ...
            if ( tour.getTourCategory().equalsIgnoreCase(ModelStructure.JOINT_NON_MANDATORY_CATEGORY) ){
                ...
                for ( int n=0; n < personNums.length; n++ ){
                    ...
                    choiceModelDescription = String.format(
                        ...
                        tourCategory, purposeName, household.getHhMgra(), tour.getTourDestMgra());
                    decisionMakerLabel = String.format("HH=%d, person record %d of %d in joint tour, PersonNum=%d, PersonType=%s, TourId=%d",
                            person.getHouseholdObject().getHhId(), p, personNums.length, person.getPersonNum(), person.getPersonType(), tour.getTourId());
        ...
        logsumHelper.setTourMcDmuAttributes( mcDmuObject, tour.getTourOrigMgra(), tour.getTourDestMgra(), tour.getTourDepartPeriod(), tour.getTourArrivePeriod(), (mcDmuObject.getDmuIndexValues().getDebug() && DEBUG_BEST_PATHS) );
        ...
        else
        {
            ...
            if ( tour.getTourCategory().equalsIgnoreCase(ModelStructure.JOINT_NON_MANDATORY_CATEGORY) ){
                ...
                for ( int n=0; n < personNums.length; n++ ){
                    ...
                    choiceModelDescription = String.format(
                        "No alternatives available for %s Tour Mode Choice Model for: Purpose=%s, Home=%d, Dest=%d",
                        tourCategory, purposeName, household.getHhMgra(), tour.getTourDestMgra());
                    decisionMakerLabel = String.format("HH=%d, person record %d of %d in joint tour, PersonNum=%d, PersonType=%s, TourId=%d",
                            person.getHouseholdObject().getHhId(), p, personNums.length, person.getPersonNum(), person.getPersonType(), tour.getTourId());
                    loggingHeader = String.format("%s    %s", choiceModelDescription, decisionMakerLabel);
                ...
                choiceModelDescription = String.format(
                    "No alternatives available for %s Tour Mode Choice Model for: Purpose=%s, Orig=%d, Dest=%d",
                    tourCategory, purposeName, tour.getTourOrigMgra(), tour.getTourDestMgra());
                decisionMakerLabel = String.format("HH=%d, PersonNum=%d, PersonType=%s, TourId=%d",
                        person.getHouseholdObject().getHhId(), person.getPersonNum(), person.getPersonType(), tour.getTourId());
                loggingHeader = String.format("%s    %s", choiceModelDescription, decisionMakerLabel);
