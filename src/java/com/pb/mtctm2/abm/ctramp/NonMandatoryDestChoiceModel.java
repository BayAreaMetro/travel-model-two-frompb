...
import com.pb.mtctm2.abm.ctramp.DestinationSampleOfAlternativesModel;
...
public class NonMandatoryDestChoiceModel implements Serializable {
    ...
    // set priority ranking for non-mandatory purposes - include 0 values for 0 element and mandatory purposes
    private static final int[] tourPurposePriorities = {
        0,
        0,
        0,
        0,
    ...
    private static final String[] dcModelSheetKeys = {
        ...
        PROPERTIES_DC_SHOP_MODEL_SHEET,
        PROPERTIES_DC_MAINT_MODEL_SHEET,
        PROPERTIES_DC_EATOUT_MODEL_SHEET,
        PROPERTIES_DC_VISIT_MODEL_SHEET,
        PROPERTIES_DC_DISCR_MODEL_SHEET
    };

    private static final String[] dcSoaModelSheetKeys = {
        PROPERTIES_DC_SOA_ESCORT_MODEL_SHEET,
        PROPERTIES_DC_SOA_SHOP_MODEL_SHEET,
        PROPERTIES_DC_SOA_MAINT_MODEL_SHEET,
        PROPERTIES_DC_SOA_EATOUT_MODEL_SHEET,
        PROPERTIES_DC_SOA_VISIT_MODEL_SHEET,
    ...
    {
        { -1.065820, -0.871051, -1.439514 },
        { -0.467154, -1.411351, -2.044826 },
        { -0.941865, -0.813977, -1.789714 },
        { -1.007316, -0.968856, -1.365375 },
        { -1.081531, -1.121260, -1.093461 },
        { -1.258919, -1.155085, -0.913773 }
    };
    ...
    public NonMandatoryDestChoiceModel( HashMap<String, String> propertyMap, ModelStructure myModelStructure,
    ...
    private void setupDestChoiceModelArrays( HashMap<String, String> propertyMap, CtrampDmuFactoryIf dmuFactory ) {
        ...
        for (int uecIndex : modelIndexMap.keySet() )
        {
            ...
            {
                logger.error( String.format("exception caught setting up DC ChoiceModelApplication[%d] for model index=%d of %d models", i, i, modelIndexMap.size()) );
    ...
    public void applyIndivModel( Household hh ) {
        ...
        for ( i=1; i < persons.length; i++ ) {
            ...
            ArrayList<Tour> tourList = getPriorityOrderedTourList( p.getListOfIndividualNonMandatoryTours() );
            ...
            for ( Tour tour : tourList ) {
                ...
                try {
                    ...
                    // for individual non-mandatory DC, just count remaining individual non-mandatory tours
                    ...
                    logger.fatal( String.format("exception caught selecting individual non-mandatory tour destination choice for hh.hhid=%d, personNum=%d, tourId=%d, purposeName=%s", hh.getHhId(), p.getPersonNum(), tour.getTourId(), tour.getTourPurpose() ) );
                    logger.fatal( "Exception caught:", e );
                    logger.fatal( "Throwing new RuntimeException() to terminate." );
                    throw new RuntimeException();
                }
    ...
    public void applyJointModel( Household hh ) {
        ...
        for ( Tour tour : tourList ) {
            ...
            try {
                ...
                logger.fatal( String.format("exception caught selecting joint non-mandatory tour destination choice for hh.hhid=%d, tourId=%d, purposeName=%s", hh.getHhId(), tour.getTourId(), tour.getTourPurpose() ) );
                logger.fatal( "Exception caught:", e );
                logger.fatal( "Throwing new RuntimeException() to terminate." );
                throw new RuntimeException();
            }
    ...
    private int selectLocationFromSampleOfAlternatives(Tour tour, DestChoiceDMU dcDmuObject, DcSoaDMU dcSoaDmuObject, TourModeChoiceDMU mcDmuObject)
    {
        ...
        // get the Household object for the person making this non-mandatory tour
        ...
        //double[] homeMgraDistanceArray = mandAcc.calculateDistancesForAllMgras( household.getHhMgra() );
        mcModel.getAnmSkimCalculator().getOpSkimDistancesFromMgra( household.getHhMgra(), mgraDistanceArray );
        ...
        // set the destAltsSample array to 1 for all destination choice alternatives
        // for each purpose
   ...
   private void setModeChoiceDmuAttributes(Household household, Person person, Tour t, int startPeriod, int endPeriod, int sampleDestMgra)
    ...
    private double calculateSimpleTODChoiceLogsum(Person person, Tour tour, int sampleDestMgra, int sampleNum)
    {
        ...
        if (household.getDebugChoiceModels()) {
            choiceModelDescription = String.format(
                    "Non-Mandatory Simplified TOD logsum calculations for %s Location Choice, Sample Number %d", tour.getTourPurpose(), sampleNum);
            decisionMakerLabel = String.format(
                    "HH=%d, PersonNum=%d, PersonType=%s, tourId=%d of %d non-mand tours", household.getHhId(), person
                            .getPersonNum(), person.getPersonType(), tour.getTourId(), person.getListOfIndividualNonMandatoryTours().size() );
    ...
    */
    ...
    public static void main( String[] args )
    {
        ...
        HouseholdIndividualNonMandatoryTourFrequencyModel inmtfModel = new HouseholdIndividualNonMandatoryTourFrequencyModel(propertyMap,
                dmuFactory, aggAcc.getAccessibilitiesTableObject(), mandAcc);

        TourModeChoiceModel inmmcModel = new TourModeChoiceModel
            ( propertyMap, modelStructure, "Non-Mandatory", dmuFactory, logsumHelper );
        
        NonMandatoryDestChoiceModel testObject = new NonMandatoryDestChoiceModel( propertyMap, modelStructure, aggAcc, dmuFactory, inmmcModel );
