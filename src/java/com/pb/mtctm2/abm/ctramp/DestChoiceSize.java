        ...
        implements Serializable
{
    ...
    // 1st dimension is an index for the set of DC Size variables used in Sample of
    ...
    /**
     * @return a boolean for whether or not the size terms in this object have been calculated
     */
    ...
    public void balanceSizeVariables(int[][] originsByHomeMgra)
    {
        ...
        // create the total origin locations array to store total tours by segment
        double[] totalOriginLocations = new double[numSegments];

        // create the total destination choice size array to store total tours by
        // segment
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            logger.info(String.format("    %-6d  %-24s:  %10.1f", i, segmentString, totalOriginLocations[i]));
        } // i
        logger.info(String.format("    %-6s  %-24s:  %10.1f", " ", "Total", segmentSum));
        logger.info("");
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            logger.info(String.format("    %-6d  %-24s:  %10.1f", i, segmentString, totalDestSize[i]));
        }
        logger.info(String.format("    %-6s  %-24s:  %10.1f", " ", "Total", segmentSum));
        logger.info("");

        // save original adjusted size variable arrays prior to balancing - used in
        // reporting size variable calculations to output files.
        originalAdjSize = duplicateDouble2DArray(balanceSize);

        // Balance destination choice size variables to equal total origin locations by segment.
        // The scaledSize calculated is what is adjusted by shadow pricing
        // adjustments, and dcSize, the array referenced
        // by UEC DMUs is a duplicate copy of this array after the shadow pricing
        ...
        // set destination choice size variables for the first iteration of shadow
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            logger.info(String.format("    %-6d  %-24s:  %10.1f", i, segmentString, sumScaled[i]));
        }
        logger.info(String.format("    %-6s  %-24s:  %10.1f", " ", "Total", segmentSum));
        logger.info("");

        // save scaled size variables used in shadow price adjustmnents for reporting
        // to output file
    ...
    public void reportMaxDiff(int iteration, int[][] modeledDestinationLocationsByDestMgra)
    {
        ...
        for (int r = 0; r < maxSize.length; r++)
        {
            ...
            logger.info(String.format("%-6s  %-24s %15s %15s %15s %15s %15s %15s %15s %8s", "index", "segment",
            ...
            for (int i = 0; i < numSegments; i++)
            {
                ...
                for (int j = 1; j <= maxMgra; j++)
                {
                    ...
                    if (scaledSize[i][j] > minRange && scaledSize[i][j] <= maxSize[r])
                    {
                        ...
                        {
                            // store the number of DC alternatives where DC Size > 0,
                        ...
                        } else
                        {
                            ...
                            for (int k = 0; k < maxDeltas.length; k++)
                            {
                                ...
                                {
                                    // store number of DC alternatives chosen where
                ...
                String segmentString = segmentIndexNameMap.get(i);
                logRecord = String.format("%-6d  %-24s", i, segmentString);
            ...
            logRecord = String.format("%-6s  %-24s", " ", "Total");
    ...
    public void saveSchoolMaxDiffValues(int iteration, int[][] modeledDestinationLocationsByDestMgra)
    {
        ...
        HashMap<Integer, Integer>  gsDistricts = new HashMap<Integer, Integer>();
        HashMap<Integer, Integer>  hsDistricts = new HashMap<Integer, Integer>();
        for (int i=0; i < mgraManager.getMgras().size(); i++) {            
            int gsDistrict = mgraManager.getMgraGradeSchoolDistrict(mgraManager.getMgras().get(i));
            int hsDistrict = mgraManager.getMgraHighSchoolDistrict(mgraManager.getMgras().get(i));
            gsDistricts.put(gsDistrict, gsDistrict);
            hsDistricts.put(hsDistrict, hsDistrict);
        }
        int gschooldistricts = gsDistricts.size();
        int hschooldistricts = hsDistricts.size();
        //1 preschool segment and 2 univ segments
        int[] segmentRange = { 0, gschooldistricts, gschooldistricts+hschooldistricts, gschooldistricts+hschooldistricts+2 }; 
    ...
    public void restoreShadowPricingInfo(String fileName)
    {
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            // first restore the scaled size values; getColumnAsFloat(column) takes a
