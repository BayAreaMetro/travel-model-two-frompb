...
import com.pb.mtctm2.abm.ctramp.Definitions;
        ...
        implements Serializable
{
    ...
    private void setUpModels(HashMap<String, String> propertyMap, CtrampDmuFactoryIf dmuFactory)
    {
        ...
        // choiceModelApplication to get its uec and from it, get the TableDataSet
        // of alternatives
        // to use to determine which tour purposes should be generated for the chose
        // alternative.
        ...
        // check the field names in the alternatives table; make sure the their order
        // is as expected.
        String[] fieldNames = alternativesTable.getColumnLabels();

        // create a mapping between names used in lookup file and purpose names used
        // in model
    ...
    public void applyModel(Household household)
    {
        ...
        // this will be an array with values 1 -> tours.length being the number of
        // non-mandatory tours in each category
        // this keeps it consistent with the way the alternatives are held in the
    ...
    private String getAlternativeNameFromChoice(int choice)
    {
        ...
        // alternative number,
        // and subsequent fields indicate the number of tours to be generated for the
    ...
    private String getAlternativeNameFromModifiedChoice(float[] rowValues)
    {
        ...
        // rowValues is a 0s based indexed array, but the first field is the
        // alternative number,
        // and subsequent fields indicate the nuimber of tours to be generated for
    ...
    {
        // this array matches the alternative table's structure, with the first row
        // being an alternative
        // could possibly look for the alternative with the largest sum, but this
    ...
    private float[] runIndividualNonMandatoryToursIncreaseModel(Person person, int choice)
    {
        ...
        while(notDone)
        {
            ...
            if (firstCount == 0 || firstCount >= 5) // if 0 or 5+ tours already, we
                // are done
                break;
            ...
            for (int i = 1; i < rowValues.length; i++)
            {
                ...
                if (household.getDebugChoiceModels())
                {
                    ...
                    // probabilities array has tour frequency extension probabilities alread stored as cumulative probabilities.
                    // calculate alternative probabilities from cumulative for logging.
    ...
    // // rowValues is a 0s based indexed array, but the first field is the
    // alternative number,
    // // and subsequent fields indicate the nuimber of tours to be generated for the
    // purpose corresponding to the field.
    // for ( int i=1; i < rowValues.length; i++ ) {
    //
    // int numTours = (int)rowValues[i];
