        ...
        implements HouseholdDataManagerIf, Serializable
{
    ...
    protected double[] percentHhsIncome75Kplus;
    ...
    public void resetCdapRandom()
    {
        for (int i = 0; i < hhs.length; i++)
        {
            ...
            int cdapCount = hhs[i].getFpRandomCount();
    ...
    public double[] getPercentHhsIncome75Kplus() {
        return percentHhsIncome75Kplus;
    }
    ...
    public void computeTransponderChoiceTazPercentArrays() {
        ...
        // for percent of households in TAZ with income > $75K
        percentHhsIncome75Kplus = new double[maxTaz+1];
        
        // for percent of households in TAZ with multiple autos
        ...
        out.println( "taz,numHhsTaz,numHhsIncome100KplusTaz,numHhsIncome75KplusTaz,numHhsMultipleAutosTaz,proportionHhsIncome100KplusTaz,proportionHhsIncome75KplusTaz,proportionHhsMultipleAutosTaz" );
        
        for ( int j=0; j < tazManager.getTazs().length; j++){
            
        	int i = tazManager.getTazs()[j];
            out.print( i + "," + numHhs[i] + "," + percentHhsIncome100Kplus[i] + "," + percentHhsIncome75Kplus[i] + "," + percentHhsMultipleAutos[i] );
            if ( numHhs[i] > 0 ) {
                percentHhsIncome100Kplus[i] /= numHhs[i]; 
                percentHhsIncome75Kplus[i] /= numHhs[i];
                percentHhsMultipleAutos[i] /= numHhs[i]; 
                out.println( "," + percentHhsIncome100Kplus[i] + "," + percentHhsIncome75Kplus[i] + "," + percentHhsMultipleAutos[i] );
            }
            else {
                out.println( "," + 0.0 + "," + 0.0 );
            }
