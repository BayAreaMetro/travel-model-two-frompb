...
public class VisitorTripModeChoiceModel {
	 ...
	 */
	public VisitorTripModeChoiceModel(HashMap<String, String> propertyMap, VisitorModelStructure myModelStructure, VisitorDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper ){
	 ...
	 * @param dmuFactory
	 */
    private void setupTripModeChoiceModel( HashMap<String, String> propertyMap, VisitorDmuFactoryIf dmuFactory  ) {
     ...
     * 
     * @param tour VisitorTour 
     * @param trip VisitorTrip 
     * 
      */
    ...
    public void setDmuAttributes(VisitorTour tour, VisitorTrip trip){
    	...
    	if(modelStructure.getTourModeIsSov(tourMode))
    		dmu.setTourModeIsDA(1);
    	else
       		dmu.setTourModeIsDA(0);
    	    		
    	if(modelStructure.getTourModeIsS2(tourMode))
    		dmu.setTourModeIsS2(1);
    	else
    		dmu.setTourModeIsS2(0);
    		
    	if(modelStructure.getTourModeIsS3(tourMode))
    		dmu.setTourModeIsS3(1);
    	else
    		dmu.setTourModeIsS3(0);

    	if(modelStructure.getTourModeIsWalk(tourMode))
    		dmu.setTourModeIsWalk(1);
    	else
    		dmu.setTourModeIsWalk(0);

    	if(modelStructure.getTourModeIsBike(tourMode))
    		dmu.setTourModeIsBike(1);
    	else
    		dmu.setTourModeIsBike(0);

    	if(modelStructure.getTourModeIsWalkTransit(tourMode))
    		dmu.setTourModeIsWalkTransit(1);
    	else
    		dmu.setTourModeIsWalkTransit(0);

       	if(modelStructure.getTourModeIsPnr(tourMode))
    		dmu.setTourModeIsPNRTransit(1);
    	else
    		dmu.setTourModeIsPNRTransit(0);

       	if(modelStructure.getTourModeIsKnr(tourMode))
    		dmu.setTourModeIsKNRTransit(1);
    	else
    		dmu.setTourModeIsKNRTransit(0);

       	if(modelStructure.getTourModeIsTaxi(tourMode))
    		dmu.setTourModeIsTaxi(1);
    	else
    		dmu.setTourModeIsTaxi(0);
