...
/**
 * This class is the TOD choice model for cross border tours.  It is currently based on a static probability distribution
...
public class VisitorTourTimeOfDayChoiceModel {
	...
	public VisitorTourTimeOfDayChoiceModel(HashMap<String,String> rbMap ){
        ...
        String stationDiurnalFile = Paths.get(directory,Util.getStringValueFromPropertyMap(rbMap,"visitor.tour.tod.file")).toString();
	 ...
	 * 
	 * @param tour A cross border tour (with purpose)
	 */
	public void calculateTourTOD(VisitorTour tour){
		...
		if(tour.getDebugChoiceModels()){
			logger.info("Choosing tour time of day for purpose "+modelStructure.VISITOR_PURPOSES[purpose]+" using random number "+random);
		...
		if(tour.getDebugChoiceModels()){
			logger.info("");
			logger.info("Chose depart period "+ tour.getDepartTime()+" and arrival period "+tour.getArriveTime());
			logger.info("");
		}
	}
