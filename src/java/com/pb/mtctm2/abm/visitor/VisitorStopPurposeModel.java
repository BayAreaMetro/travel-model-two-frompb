...
/**
 * This class is the stop purpose choice model for visitor tours.  It is currently based on a static probability distribution
 * stored in an input file, and indexed into by purpose, tour leg direction (inbound or outbound), the stop number, and whether there
 * is just one or multiple stops on the tour leg.
 * 
 * @author Freedman
 *
 */
public class VisitorStopPurposeModel {
	   ...
	   HashMap<Integer,Integer> arrayElementMap;  // Hashmap used to get the element number of the cumProbability array based on the 
	...
	private void readStopPurposeFile(String fileName){
		...
		for(int row=1;row<=rowCount;++row){
	    	...
	    	if(Math.abs(cumProb - 1.0)> 0.00001)
	    			logger.info("Cumulative probability for tour purpose "+purpose+" inbound "+inbound+" stopNumber "+stopNumber+
	    					" multiple "+multiple+" is "+cumProb);
	 ...
	 * 
	 * @param tourPurp   Tour purpose
	 * @param isInbound  1 if the stop is on the inbound direction, else 0.
	 * @param stopNumber The number of the stop.
	 * @param multipleStopsOnLeg 1 if multiple stops on leg, else 0.
	 ...
	 * 
	 * @param tour A cross border tour (with tour purpose)
	 */
	 ...
	 * @param random  Uniformly distributed random number
	 * @param cumProb Cumulative probability distribution
