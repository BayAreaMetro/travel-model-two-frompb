...
/**
 * This class is used for both the sample of alternatives and the full destination choice model for visitor tours.
 * 
 *  
 * @author Freedman
 *
...
public class VisitorTourDestChoiceModel {
	...
	private int[] sampleMgras;                      //numbers of mgra for the sample
	private int[] sampleTazs;                      //numbers of taz for the sample
	private double[] sampleCorrectionFactors;		//correction factors for sample
	private double[][] sampleSizeTerms;               //size terms for sample
	private double[]  sampleLogsums;                 //tour mc logsums
	...
	private HashMap<Integer, Integer> frequencyChosen;                //by mgra, number of times chosen
     ...
     * Constructor 
     * 
     * @param propertyMap  Resource properties file map.
     * @param dmuFactory   Factory object for creation of airport model DMUs
     */
	public VisitorTourDestChoiceModel(HashMap<String,String> rbMap, VisitorModelStructure modelStructure, VisitorDmuFactoryIf dmuFactory, 
        ...
        // read the model pages from the property file, create one choice model for each soa model
        ...
        for(int i = 0; i < soaModel.length; ++i){
            ...
            soaModel[i] = new ChoiceModelApplication( visitorDCSoaFileName, destModelPage, dataPage, rbMap, (VariableTable)dcDmu );
        }
        ...
        sizeTermUEC = new UtilityExpressionCalculator(new File(visitorDCSoaFileName), sizePage, dataPage, rbMap, (VariableTable)dcDmu);
        ...
        for(int i = 0; i < destModel.length; ++i){
            ...
            // create a ChoiceModelApplication object for the filename, model page and data page.
            destModel[i] = new ChoiceModelApplication( visitorDCFileName, destModelPage, dataPage, rbMap, (VariableTable)dcDmu );
            if(i==0)
            	sampleRate = destModel[i].getNumberOfAlternatives();
        }
        ...
        tourModeChoiceModel = new VisitorTourModeChoiceModel(rbMap, modelStructure,
	            dmuFactory,  logsumsCalculator);
           		...
           		for(int mgra = 0; mgra < mgraArray.length; ++ mgra){
        			...
        			int mgraNumber = mgraArray[mgra];
           			if(tazSizeTerms[purpose][tazNumber]>0.0)
           				mgraProbabilities[purpose][tazNumber][mgra] = lastProb + mgraSizeTerms[purpose][mgraNumber]/tazSizeTerms[purpose][tazNumber];
           			lastProb = mgraProbabilities[purpose][tazNumber][mgra];
           		}
           		if(tazSizeTerms[purpose][tazNumber]>0.0 && Math.abs(lastProb-1.0)> 0.000001)
           			logger.info("Error: purpose " + purpose + " taz "+tazNumber+" cum prob adds up to "+lastProb);
        	}
	...
	/**
	 * Calculate taz probabilities.  This method initializes and calculates
	 * the tazProbabilities array.
	 */
	public void calculateTazProbabilities(VisitorDmuFactoryIf dmuFactory){
        ...
        TableDataSet altData = soaModelUEC.getAlternativeData();
     	...
     	for(int purpose = 0; purpose<soaModel.length;++purpose){
    		...
    		tazProbabilities[purpose] = new Matrix("Prob_Matrix","Probability Matrix",altData.getRowCount()+1,altData.getRowCount()+1);
	 ...
	 * @param tour VisitorTour with purpose and Random  
	 */
	private void chooseMgraSample(VisitorTour tour){
		...
		for(int sample=1;sample<=sampleRate;++sample){
			...
			for(int i=0; i < tazCumProb.getColumnCount(); ++ i){
				...
				if(tazCumProb.getValueAt(originTaz, destinationTaz) > random){
					alt = i;
					if(i != 0){
						cumProb = tazCumProb.getValueAt(originTaz, tazCumProb.getExternalColumnNumber(i-1));
						altProb = tazCumProb.getValueAt(originTaz, destinationTaz)-tazCumProb.getValueAt(originTaz, tazCumProb.getExternalColumnNumber(i-1));
					}else{
		...
		for(int sample = 1;sample<=sampleRate;++sample){
			...
			sampleCorrectionFactors[sample] = (float) Math.log((double) freq / sampleCorrectionFactors[sample]);
	...
	private void calculateLogsumsForSample(VisitorTour tour){
		...
		for(int sample = 1; sample <=sampleRate;++sample){
			...
			if(sampleMgras[sample]>0){
				...
				double logsum = tourModeChoiceModel.getModeChoiceLogsum(tour, logger, "Sample logsum "+sample, "tour "+tour.getID()+" dest "+destinationMgra);
				sampleLogsums[sample]=logsum;
			}
			else
				sampleLogsums[sample]=0;
	 ...
	 * 
	 * @param tour A cross border tour with a tour origin, purpose, attributes, and departure\arrival time and SENTRI availability members.
	 */
	public void chooseDestination(VisitorTour tour){
		
		//reset the origin TAZ in the DMU
		int originTaz = mgraManager.getTaz(tour.getOriginMGRA());
        ...
        if(tour.getDebugChoiceModels()){
        	...
        	//log the sample
        	destModel[purpose].choiceModelUtilityTraceLoggerHeading("Visitor tour destination model",
                    "tour "+tour.getID());
        }
	...
	/**
	 * @param tourModeChoiceModel the tourModeChoiceModel to set
	 */
	public void setTourModeChoiceModel(
			VisitorTourModeChoiceModel tourModeChoiceModel) {
