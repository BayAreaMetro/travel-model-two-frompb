...
/**
 * This class is the TOD choice model for visitor tours.  It is currently based on a static probability distribution
 * stored in an input file, and indexed into by purpose.
 * 
 * @author Freedman
 *
 */
public class VisitorStopTimeOfDayChoiceModel {
	    ...
	    private HashMap<Integer,Integer> outboundElementMap;  // Hashmap used to get the element number of the cumProbability array based on the 
           //  tour duration and stop number.
	
	    private HashMap<Integer,Integer> inboundElementMap;  // Hashmap used to get the element number of the cumProbability array based on the 
        // tour duration and stop number.
	...
	/**
	 * Read the outbound stop duration file and store the cumulative probability distribution
	 * as well as the offsets and set the key map to index into the probability array.
	 * @param fileName
	 */
	...
	/**
	 * Read the inbound stop duration file and store the cumulative probability distribution
	 * as well as the offsets and set the key map to index into the probability array.
	 * @param fileName
	 */
	 ...
	 * @param periodsRemaining  Remaining time periods
	...
	public void chooseTOD(VisitorTour tour, VisitorStop stop){
		...
		if(!inbound){
			...
			if(tour.getDebugChoiceModels()){
				logger.info("Stop TOD Choice Model for tour "+tour.getID()+" outbound stop "+stop.getId()+" periods remaining "+periodsRemaining);
			...
			for(int i =0;i<cumProb.length;++i){
				if(random<cumProb[i]){
					...
					if(tour.getDebugChoiceModels()){
						logger.info("***");
						logger.info("Chose alt "+i+" offset "+offset+" from depart period "+departPeriod);
		...
		//inbound stop	
			...
			double random = tour.getRandom();
			if(tour.getDebugChoiceModels()){
				logger.info("Stop TOD Choice Model for tour "+tour.getID()+" inbound stop "+stop.getId()+" periods remaining "+periodsRemaining);
				logger.info("Random number "+random);
			}			
			for(int i =0;i<cumProb.length;++i){
				if(random<cumProb[i]){
					...
					if(tour.getDebugChoiceModels()){
						logger.info("***");
						logger.info("Chose alt "+i+" offset "+offset+" from arrive period "+arrivePeriod);
						logger.info("Stop period is "+stop.getStopPeriod());
