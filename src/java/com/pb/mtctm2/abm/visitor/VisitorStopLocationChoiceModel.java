...
public class VisitorStopLocationChoiceModel {
	...
	private TableDataSet alternativeData;    //the alternatives, with a "dest" - indicating the destination TAZ in San Diego County  	
	...
	private int originMgra;      // the origin MGRA of the stop (originMgra -> stopMgra -> destinationMgra)
	private int destinationMgra; // the destination MGRA of the stop (originMgra -> stopMgra -> destinationMgra)
	 ...
	 */
	public VisitorStopLocationChoiceModel(HashMap<String, String> propertyMap, VisitorModelStructure myModelStructure, VisitorDmuFactoryIf dmuFactory, McLogsumsCalculator myLogsumHelper){
	 ...
	 * @param dmuFactory
	 */
    private void setupStopLocationChoiceModel( HashMap<String, String> rbMap, VisitorDmuFactoryIf dmuFactory  ) {
        ...
        destModel = new ChoiceModelApplication( visitorStopLocationFileName, modelPage, dataPage, rbMap, (VariableTable)dmu );
    ...
    private void createSample(VisitorTour tour, VisitorStop stop){
    	...
    	int origTaz = 0;
       	int destTaz = 0; 
    	int period = modelStructure.AM + 1;
    	...
    	//distances
    	soaTourOrigToStopDistanceAlt = logsumHelper.getAnmSkimCalculator().getTazDistanceFromTaz(origTaz, period);
    	soaStopToTourDestDistanceAlt = logsumHelper.getAnmSkimCalculator().getTazDistanceToTaz(destTaz, period);
    	...
    	for(int sample = 1; sample<=sampleRate;++sample){
			...
			if(mgraCumProb == null){
				logger.error("Error: mgraCumProb array is null for purpose "+purpose+" sampledTaz "+sampledTaz+" hhID "+tour.getID());
    ...
    public void chooseStopLocation(VisitorTour tour, VisitorStop stop){
        ...
        {
        	String decisionMakerLabel = "Tour ID "+tour.getID()+" stop id "+stop.getId()+" purpose "+modelStructure.VISITOR_PURPOSES[stop.getPurpose()];
        	destModel.choiceModelUtilityTraceLoggerHeading("Intermediate stop location choice model", decisionMakerLabel);
        }
        ...
        {
           	String decisionMakerLabel = "Tour ID "+tour.getID()+" stop id "+stop.getId()+" purpose "+modelStructure.VISITOR_PURPOSES[stop.getPurpose()];
            String loggingHeader = String.format("%s   %s", "Intermediate stop location choice model", decisionMakerLabel);
            destModel.logUECResults(logger, loggingHeader);
            logger.info("Chose alternative "+alt+" mgra "+destMgra+" with random number "+random);
            logger.info("");
            logger.info("");
        }
