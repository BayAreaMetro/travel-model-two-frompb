...
public class VisitorTourEstimationFile {
	...
	public void createEstimationFile(){
		...
		logsumsCalculator.setTazDistanceSkimArrays( tazDistanceCalculator.getStoredFromTazToAllTazsDistanceSkims(), tazDistanceCalculator.getStoredToTazFromAllTazsDistanceSkims() );
		
		tourModeChoiceModel = new VisitorTourModeChoiceModel(rbMap, myModelStructure,
	 ...
	 * @param data  TableDataSet to write
	 */
	 ...
	 * 
	 * TOURNUM: Unique ID 
	 * SURVNUM: Number associated with traveler
	 * PersonType: 0 - Business, 1 - Personal
	 * Persons: Number of persons (in addition to traveler) on tour
	 * HHIncome: 1= $0-29,999; 2 = 30,000-59,999; 3 = 60,000-99,999; 4 = 100,000-149,000; 5=$150,000 or more; 6 = DK/Refused
	 * AutoAvail: 0 - No auto, 1 - Auto Available
	 * PURPOSE: 1 - Work, 2 - Other (non-other) 3 - Other
	 * originMGRA - origin of tour (hotel/overnight location)
	 * destMGRA - primary destination MGRA
	 * PeriodDepart: Period of Tour Departure from hotel/overnight location
	 * PeriodArrive: Period of Tour Arrival at primary destination
	 * SAMPLE_: 1:30 sampled alternatives 
	 * 
	 */
	public void calculateTourMCLogsums(){
		...
		for(int i = 0;i<estimationData.getRowCount();++i){
			...
			if((i+1)==1||(i+1)==500)
				...
				tour.setDebugChoiceModels(false);
			...
			for(int j = 0;j<SAMPLE_SIZE;++j){
				...
				if(sample[j]>0 && originMGRA>0){
					logsum = tourModeChoiceModel.getModeChoiceLogsum(tour, logger, "DCEstimationFileLogsum", "ID "+ID);
	...
	public static void main(String[] args) {
        ...
        try
        {
            ...
            if (!matrixServerAddress.equalsIgnoreCase("none"))
            {
                ...
                if (matrixServerAddress.equalsIgnoreCase("localhost")) {
                    matrixServer = visitorEstimationFile.startMatrixServerProcess(matrixServerAddress, serverPort, mt);
                    visitorEstimationFile.ms = matrixServer;
                }
                else {
                	visitorEstimationFile.ms = new MatrixDataServerRmi( matrixServerAddress, serverPort, MatrixDataServer.MATRIX_DATA_SERVER_NAME );
		...
		if(args.length<3){
			System.out.println("Error: please specifiy inputFileName and outputFileName on command line");
			throw new RuntimeException();
		}
	   ...
	   private MatrixDataServerRmi startMatrixServerProcess( String serverAddress, int serverPort, MatrixType mt )
	    {
