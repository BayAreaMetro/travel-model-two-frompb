        ...
        extends HouseholdDataManager
{
    ...
    public void mapTablesToHouseholdObjects()
    {
        ...
        // array.
        // the hhIndex array will store the hhArray index number for the given hh
        // index.
        int maxHhId = 0;
        ...
        for (int r = 1; r <= hhTable.getRowCount(); r++)
        {
            ...
            try
            {
                ...
                int numAutos = (int) hhTable.getValueAt(r, hhTable.getColumnPosition(HH_AUTOS_FIELD_NAME));
                hh.setHhAutos(numAutos);

                // set the hhSize variable and create Person objects for each person
                int numPersons = (int) hhTable.getValueAt(r, hhTable.getColumnPosition(HH_SIZE_FIELD_NAME));
                ...
                // group quarters
                int unitType = (int) hhTable.getValueAt(r, hhTable.getColumnPosition(HH_UNITTYPE_FIELD_NAME));
                ...
                int type = (int) hhTable.getValueAt(r, hhTable.getColumnPosition(HH_TYPE_FIELD_NAME));
                hh.setHhType(type);
        ...
        for (int r = 1; r <= personTable.getRowCount(); r++)
        {
            ...
            try
            {
                ...
                persId = (int) personTable.getValueAt(r, personTable.getColumnPosition(PERSON_PERSON_ID_FIELD_NAME));
                ...
                int age = (int) personTable.getValueAt(r, personTable.getColumnPosition(PERSON_AGE_FIELD_NAME));
                person.setPersAge(age);
                fieldCount++;

                int gender = (int) personTable.getValueAt(r, personTable.getColumnPosition(PERSON_GENDER_FIELD_NAME));
                person.setPersGender(gender);
                fieldCount++;              


                int pecasOcc = (int)personTable.getValueAt( r, personTable.getColumnPosition( PERSON_OCCP_FIELD_NAME ) );
                ...
                int schg = (int)personTable.getValueAt( r, personTable.getColumnPosition(PERSON_GRADE_ATTENDING_FIELD_NAME));
                ...
                // and
                // beyond)
                int educ = (int) personTable.getValueAt(r, personTable.getColumnPosition(PERSON_EDUCATION_ATTAINMENT_FIELD_NAME));
                if (educ >= 9) person.setPersonIsHighSchoolGraduate(true);
                else person.setPersonIsHighSchoolGraduate(false);
                fieldCount++;

                // Person educational attainment level to determine higher education
                ...
                int grade = (int) personTable.getValueAt(r, personTable.getColumnPosition(PERSON_GRADE_ATTENDING_FIELD_NAME));
