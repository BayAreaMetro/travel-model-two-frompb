...
public class SandagSummitFile {
    ...
    //Some parameters
    private int modes;
    private int upperEA;  // Upper limit on time of day for the Early morning time period
    private int upperAM;  // Upper limit on time of day for the AM peak time period
    private int upperMD;  // Upper limit on time of day for the Midday time period
    private int upperPM;  // Upper limit on time of day for the PM time peak period
	...
	private final static String purposeName[] = {"Work","University","School","Escort","Shop","Maintenance","EatingOut","Visiting", "Discretionary", "WorkBased"};
	...
	public SandagSummitFile(String resourceFile){
		...
		while(inToken.hasMoreElements()){
			...
			if(modeValue==1)
				walkTransitModes[mode]=1;
			else if(modeValue==2)
				driveTransitModes[mode]=1;
	...
	/**
	 * Create Summit files for all purposes and both individual and joint tour files.
	 * 
	 */
	 ...
	 * 
	 * @param fileName household file path/name.
	 */
	 ...
	 * 
	 * @param fileName  Person file path/name.
	 */
	...
	/**
	 * This is the main workhorse method in this class.  It creates a SummitRecordTable, and
	 * then iterates over records in the tour file.  If the tour purpose for the record
	 * is mapped to the fileNumber argument, a ConcreteSummitRecord is created and the attributes
	 * are set, and the record is added to the SummitRecordTable.  After all tour records have
	 * been read, the table is finalized.
	 * 
	 * After this method is run, the file header information can be set and the SummitRecordTable 
	 * can be written to a SUMMIT file.
	 * 
	 * @param fileNumber
	 */
	...
	/**
	 * Calculate and return the market for the departure and arrival time periods.
	 * 
	 * Market 0 = Departure & arrival in peak
	 * Market 1 = Departure & arrival in mixed periods (peak and off-peak)
	 * Market 2 = Departure & arrival in off-peak
	 * 
	 * @param departPeriod 1-39 representing time in 30 min. increments, starting at 5 AM
	 * @param arrivePeriod 1-39 representing time in 30 min. increments, starting at 5 AM
	...
	public int calculatePeriodMarket(int departPeriod, int arrivePeriod){
		...
		if(departPeriod>upperEA && departPeriod<=upperAM)
			departPeak=1;
		else if(departPeriod>upperMD && departPeriod<=upperPM)
			departPeak=1;
		
		// check if arrival is in peak period
		if(arrivePeriod>upperEA && arrivePeriod<=upperAM)
			arrivePeak=1;
		else if(arrivePeriod>upperMD && arrivePeriod<=upperPM)
			arrivePeak=1;
		
		/* Arrival & departure = peak, period = 0
		 * Mixed peak & off-peak, period = 1
		 * Arrival & departure = off-peal, period = 2
		 */
		if(departPeak==1 && arrivePeak==1)
			return 0;
		else if(departPeak==0 && arrivePeak==0)
			return 2;
	...
	/**
	 * Calculate and return market segment based on auto sufficiency and 
	 * time period combination.
	 * 
	 * Market  AutoSuff  Period
	 * 1        0        0 Peak
	 * 2        0        1 Mixed
	 * 2        0        2 Off-Peak
	 * 3        1        0 Peak
	 ...
	 * 
	 * 
	 * @param autoSufficiency  0 = 0 autos, 1=autos < adults, 2 = autos >= adults
	 * @param periodMarket     0 = Peak, 1 = Mixed, 2 = Off-Peak
	...
	public int calculateMarketSegment(int autoSufficiency, int periodMarket){
		...
		switch (autoSufficiency){
			...
			case 2: 
				...
				break;
			default:
				logger.fatal("Error:  Could not calculate market segment auto sufficiency "+autoSufficiency);
	...
	public int calculateSummitTable(String tourPurpose){
		...
		if (tourPurpose.contentEquals("Work"))
			return fileNumber[0];
		else if (tourPurpose.contentEquals("University"))
			return fileNumber[1];
		else if (tourPurpose.contentEquals("School"))
			return fileNumber[2];
		else if (tourPurpose.contentEquals("Escort"))
			return fileNumber[3];
		else if (tourPurpose.contentEquals("Shop"))
			return fileNumber[4];
		else if (tourPurpose.contentEquals("Maintenance"))
			return fileNumber[5];
		else if (tourPurpose.contentEquals("Eating Out"))
			return fileNumber[6];
		else if (tourPurpose.contentEquals("Visiting"))
			return fileNumber[7];
		else if (tourPurpose.contentEquals("Discretionary"))
			return fileNumber[8];
		else if (tourPurpose.contentEquals("Work-Based"))
			return fileNumber[9];
		else{
	...
	/**
	 * Look up the purpose string based on the file number, for use in SUMMIT file
	 * header.
	 * 
	 * @param fileNumber
	 ...
	 * 
	 * 0 = 0 autos owned
	 * 1 = autos > 0 & autos < adults (persons 18+)
	 * 2 = autos > adults
	 * 
	...
	/**
	 * Calculate the total non-transit exponentiated utility.  The method
	 * uses the walkTransitModes array and the driveTransitModes array
	 * to determine which modes are non-transit, and the sum of their exponentiated
	 * utilities is calculated and returned.
	 * 
	 * @param util  An array of utilities, by mode. -999 indicates mode not available.
	...
	/**
	 * Calculate the share of walk-transit available:
	 *   1 if any walk-transit mode is available, as indicated by a non-zero probability,
	 *   else 0. 
	 * The method iterates through the probability array and returns a 1 if 
	 * the probability is non-zero for any walk-transit mode, as indicated by the 
	 * walkTransitModes array.
	 * 
	 * @param prob  An array of probabilities, dimensioned by modes.
	...
	public float calculateWalkTransitAvailableShare(float prob[]){
		...
		for(int i = 0; i < modes; ++ i)
			if(walkTransitModes[i] == 1 && prob[i] > 0)
				return 1.0f;
	...
	/**
	 * Calculate the share of drive-transit only available:
	 *   1 if any drive-transit mode is available, as indicated by a non-zero probability,
	 *   and all walk-transit modes are not available. 
	 * The method iterates through the probability array and returns a 1 if 
	 * the probability is non-zero for any drive-transit mode, as indicated by the 
	 * driveTransitModes array.
	 * 
	 * @param prob  An array of probabilities, dimensioned by modes.
	 * @param walkTransitAvailableShare 1 if walk-transit available, else 0.
	...
	public float calculateDriveTransitOnlyShare(float prob[], float walkTransitAvailableShare){
		...
		if(walkTransitAvailableShare>0)
			return 0.0f;
		
		//iterate through the probability array
		for(int i = 0; i < modes; ++ i)
			if(driveTransitModes[i] == 1 && prob[i] > 0)
				return 1.0f;
	...
	/**
	 * Calculate the total transit probability for records with walk-transit available.
	 * The method returns 0 if walk-transit is not available.  If walk-transit is 
	 * available, the method iterates through the probability array, adding all transit
	 * mode probabilities.  The sum is returned.
	 * 
	 * @param prob  An array of probabilities, one per mode.
	 * @param walkTransitAvailableShare  1 if walk-transit is available, else 0.
	 * @return  The total transit probability if walk-transit is available, else 0.
	 */
	...
	/**
	 * Calculate the total transit probability for records where only drive-transit available.
	 * The method returns 0 if only drive-transit is not available.  If only drive-transit is 
	 * available, the method iterates through the probability array, adding all drive-transit
	 * mode probabilities.  The sum is returned.
	 * 
	 * @param prob  An array of probabilities, one per mode.
	 * @param driveTransitOnlyAvailableShare  1 if only drive-transit is available, else 0.
	 * @return  The total transit probability if only drive-transit is available, else 0.
	 */
	public float calculateTransitShareOfDriveTransitOnly(float prob[],float driveTransitOnlyAvailableShare){
		
		float transitShare = 0.0f;
		
		//if drive-transit is unavailable, then walk-transit share is 0.
		if(driveTransitOnlyAvailableShare==0)
			return transitShare;
		
		//iterate through the probability array
		for(int i = 0; i < modes; ++ i)
			if(driveTransitModes[i] == 1) 
				transitShare += prob[i];
	...
	/**
	 * Get the in-vehicle time coefficient for the file, based
	 * on the values read in the properties file.
	 * 
	 * @param fileNumber
